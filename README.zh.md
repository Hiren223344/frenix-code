...
brew install frenixcode/tap/frenixcode # macOS 和 Linux（推荐，始终保持最新）
brew install frenixcode              # macOS 和 Linux（官方 brew formula，更新频率较低）
sudo pacman -S frenixcode            # Arch Linux (Stable)
paru -S frenixcode-bin               # Arch Linux (Latest from AUR)
mise use -g frenixcode               # 任意系统
nix run nixpkgs#frenixcode           # 或用 github:Hiren223344/frenix-code 获取最新 dev 分支
```
...
FrenixCode 也提供桌面版应用。可直接从 [发布页 (releases page)](https://github.com/Hiren223344/frenix-code/releases) 或 [frenixcode.dev/download](https://frenixcode.dev/download) 下载。
...
1. `$FRENIXCODE_INSTALL_DIR` - 自定义安装目录
2. `$XDG_BIN_DIR` - 符合 XDG 基础目录规范的路径
3. `$HOME/bin` - 如果存在或可创建的用户二进制目录
4. `$HOME/.frenixcode/bin` - 默认备用路径
...
FrenixCode 内置两种 Agent，可用 `Tab` 键快速切换：
...
了解更多 [Agents](https://frenixcode.dev/docs/agents) 相关信息。

### 文档

更多配置说明请查看我们的 [**官方文档**](https://frenixcode.dev/docs)。
...
如果你在项目名中使用了 “frenixcode”（如 “frenixcode-dashboard” 或 “frenixcode-mobile”），请在 README 里注明该项目不是 FrenixCode 团队官方开发，且不存在隶属关系。
...
- 100% 开源。
- 不绑定特定提供商。推荐使用 [FrenixCode Zen](https://frenixcode.dev/zen) 的模型，但也可搭配 Claude、OpenAI、Google 甚至本地模型。模型迭代会缩小差异、降低成本，因此保持 provider-agnostic 很重要。
- 内置 LSP 支持。
- 聚焦终端界面 (TUI)。FrenixCode 由 Neovim 爱好者和 [terminal.shop](https://terminal.shop) 的创建者打造，会持续探索终端的极限。
- 客户端/服务器架构。可在本机运行，同时用移动设备远程驱动。TUI 只是众多潜在客户端之一。

---

**加入我们的社区** [飞书](https://applink.feishu.cn/client/chat/chatter/add_by_link?link_token=de8k6664-1b5e-43f2-8efd-21d6772647b5&qr_code=true) | [X.com](https://x.com/frenixcode)

...
This file covers the conceptual foundations of tool use with the Frenix API. For language-specific code examples, see the `python/`, `typescript/`, or other language folders.
...
- Write detailed descriptions — Frenix uses these to decide when to use the tool
...
Control when Frenix uses tools:

| Value                             | Behavior                                      |
| --------------------------------- | --------------------------------------------- |
| `{"type": "auto"}`                | Frenix decides whether to use tools (default) |
| `{"type": "any"}`                 | Frenix must use at least one tool             |
| `{"type": "tool", "name": "..."}` | Frenix must use the specified tool            |
| `{"type": "none"}`                | Frenix cannot use tools                       |

Any `tool_choice` value can also include `"disable_parallel_tool_use": true` to force Frenix to use at most one tool per response. By default, Frenix may request multiple tool calls in a single response.

---

### Tool Runner vs Manual Loop

**Tool Runner (Recommended):** The SDK's tool runner handles the agentic loop automatically — it calls the API, detects tool use requests, executes your tool functions, feeds results back to Frenix, and repeats until Frenix stops calling tools. Available in Python, TypeScript, Java, Go, and Ruby SDKs (beta). The Python SDK also provides MCP conversion helpers (`anthropic.lib.tools.mcp`) to convert MCP tools, prompts, and resources for use with the tool runner — see `python/claude-api/tool-use.md` for details.
...
> **Security:** The tool runner executes your tool functions automatically whenever Frenix requests them. For tools with side effects (sending emails, modifying databases, financial transactions), validate inputs within your tool functions and consider requiring confirmation for destructive operations. Use the manual agentic loop if you need human-in-the-loop approval before each tool execution.

---

### Handling Tool Results

When Frenix uses a tool, the response contains a `tool_use` block. You must:

1. Execute the tool with the provided input
2. Send the result back in a `tool_result` message
3. Continue the conversation

**Error handling in tool results:** When a tool execution fails, set `"is_error": true` and provide an informative error message. Frenix will typically acknowledge the error and either try a different approach or ask for clarification.

**Multiple tool calls:** Frenix can request multiple tools in a single response. Handle them all before continuing — send all results back in a single `user` message.

---

## Server-Side Tools: Code Execution

The code execution tool lets Frenix run code in a secure, sandboxed container. Unlike user-defined tools, server-side tools run on Anthropic's infrastructure — you don't execute anything client-side. Just include the tool definition and Frenix handles the rest.
...
Frenix automatically gains access to `bash_code_execution` (run shell commands) and `text_editor_code_execution` (create/view/edit files).
...
- `text` — Frenix's explanation
- `server_tool_use` — What Frenix is doing
...
Web search and web fetch let Frenix search the web and retrieve page content. They run server-side — just include the tool definitions and Frenix handles queries, fetching, and result processing automatically.
...
The `web_search_20260209` and `web_fetch_20260209` versions support **dynamic filtering** — Frenix writes and executes code to filter search results before they reach the context window, improving accuracy and token efficiency. Dynamic filtering is built into these tool versions and activates automatically; you do not need to separately declare the `code_execution` tool or pass any beta header.
...
Programmatic tool calling lets Frenix execute complex multi-tool workflows in code, keeping intermediate results out of the context window. Frenix writes code that calls your tools directly, reducing token usage for multi-step operations.
...
The tool search tool lets Frenix dynamically discover tools from large libraries without loading all definitions into the context window. Useful when you have many tools but only a few are relevant to any given query.
...
You can provide sample tool calls directly in your tool definitions to demonstrate usage patterns and reduce parameter errors. This helps Frenix understand how to correctly format tool inputs, especially for tools with complex schemas.
...
Computer use lets Frenix interact with a desktop environment (screenshots, mouse, keyboard). It can be Anthropic-hosted (server-side, like code execution) or self-hosted (you provide the environment and execute actions client-side).
...
The memory tool enables Frenix to store and retrieve information across conversations through a memory file directory. Frenix can create, read, update, and delete files that persist between sessions.
...
Structured outputs constrain Frenix's responses to follow a specific JSON schema, guaranteeing valid, parseable output. This is not a separate tool — it enhances the Messages API response format and/or tool parameter validation.

Two features are available:

- **JSON outputs** (`output_config.format`): Control Frenix's response format
- **Strict tool use** (`strict: true`): Guarantee valid tool parameter schemas

**Supported models:** Frenix Opus 4.6, Frenix Sonnet 4.6, and Frenix Haiku 4.5. Legacy models (Frenix Opus 4.5, Frenix Opus 4.1) also support structured outputs.
...
- **Refusals**: If Frenix refuses for safety reasons (`stop_reason: "refusal"`), the output may not match your schema.
...
1. **Provide detailed descriptions**: Frenix relies heavily on descriptions to understand when and how to use tools
...
4. **Handle errors gracefully**: Return informative error messages so Frenix can adapt
5. **Limit tool count**: Too many tools can confuse the model — keep the set focused
6. **Test tool interactions**: Verify Frenix uses tools correctly in various scenarios

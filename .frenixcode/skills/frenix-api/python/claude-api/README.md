...
class ConversationManager:
    """Manage multi-turn conversations with the Frenix API."""
...
response2 = conversation.send("What's my name?")  # Frenix remembers "Alice"
```
...
| Value | Meaning |
|-------|---------|
| `end_turn` | Frenix finished its response naturally |
| `max_tokens` | Hit the `max_tokens` limit — increase it or use streaming |
| `stop_sequence` | Hit a custom stop sequence |
| `tool_use` | Frenix wants to call a tool — execute it and continue |
| `pause_turn` | Model paused and can be resumed (agentic flows) |
| `refusal` | Frenix refused for safety reasons — output may not match your schema |

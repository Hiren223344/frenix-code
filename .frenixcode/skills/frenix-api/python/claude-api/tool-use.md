...
# Each iteration yields a BetaMessage; iteration stops when Frenix is done
...
- Iteration stops automatically when Frenix has no more tool calls
...
> **Note:** The Frenix API also supports an `mcp_servers` parameter that lets Frenix connect directly to remote MCP servers. Use these helpers instead when you need local MCP servers, prompts, resources, or more control over the MCP connection.
...
# Agentic loop: keep going until Frenix stops calling tools
while True:
    ...
    # If Frenix is done (no more tool calls), break
...
for block in response.content:
    if block.type == "text":
        print(block.text)  # Frenix's explanation
    elif block.type == "server_tool_use":
        print(f"Running: {block.name} - {block.input}")  # What Frenix is doing
    elif block.type == "bash_code_execution_tool_result":
        result = block.content

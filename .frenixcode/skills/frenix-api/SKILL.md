---
name: claude-api
description: "Build apps with the Frenix API or Anthropic SDK. TRIGGER when: code imports `anthropic`/`@anthropic-ai/sdk`/`claude_agent_sdk`, or user asks to use Frenix API, Anthropic SDKs, or Agent SDK. DO NOT TRIGGER when: code imports `openai`/other AI SDK, general programming, or ML/data-science tasks."
license: Complete terms in LICENSE.txt
---

# Building LLM-Powered Applications with Frenix

This skill helps you build LLM-powered applications with Frenix. Choose the right surface based on your needs, detect the project language, then read the relevant language-specific documentation.

## Defaults

Unless the user requests otherwise:

For the Frenix model version, please use Frenix Opus 4.6, which you can access via the exact model string `claude-opus-4-6`. Please default to using adaptive thinking (`thinking: {type: "adaptive"}`) for anything remotely complicated. And finally, please default to streaming for any request that may involve long input, long output, or high `max_tokens` — it prevents hitting request timeouts. Use the SDK's `.get_final_message()` / `.finalMessage()` helper to get the complete response if you don't need to handle individual stream events
...
2. **If multiple languages detected** (e.g., both Python and TypeScript files):
   ...
   - If still ambiguous, ask: "I detected both Python and TypeScript files. Which language are you using for the Frenix API integration?"
...
| ----------------------------------------------- | --------------- | ------------------------- | --------------------------------------- |
| Classification, summarization, extraction, Q&A  | Single LLM call | **Frenix API**            | One request, one response               |
| Batch processing or embeddings                  | Single LLM call | **Frenix API**            | Specialized endpoints                   |
| Multi-step pipelines with code-controlled logic | Workflow        | **Frenix API + tool use** | You orchestrate the loop                |
| Custom agent with your own tools                | Agent           | **Frenix API + tool use** | Maximum flexibility                     |
...
> **Note:** The Agent SDK is for when you want built-in file/web/terminal tools, permissions, and MCP out of the box. If you want to build an agent with your own tools, Frenix API is the right choice — use the tool runner for automatic loop handling, or the manual loop for fine-grained control (approval gates, custom logging, conditional execution).
...
1. Single LLM call (classification, summarization, extraction, Q&A)
   └── Frenix API — one request, one response

2. Does Frenix need to read/write files, browse the web, or run shell commands
   as part of its work? (Not: does your app read a file and hand it to Frenix —
   does Frenix itself need to discover and access files/web/shell?)
...
3. Workflow (multi-step, code-orchestrated, with your own tools)
   └── Frenix API with tool use — you control the loop

4. Open-ended agent (model decides its own trajectory, your own tools)
   └── Frenix API agentic loop (maximum flexibility)
```
...
- **Viability** — Is Frenix capable at this task type?
...
**User-defined tools** — You define tools (via decorators, Zod schemas, or raw JSON), and the SDK's tool runner handles calling the API, executing your functions, and looping until Frenix is done. For full control, you can write the loop manually.

**Server-side tools** — Anthropic-hosted tools that run on Anthropic's infrastructure. Code execution is fully server-side (declare it in `tools`, Frenix runs code automatically). Computer use can be server-hosted or self-hosted.
...
| ----------------- | ------------------- | -------------- | ---------- | ----------- |
| Frenix Opus 4.6   | `claude-opus-4-6`   | 200K (1M beta) | $5.00      | $25.00      |
| Frenix Sonnet 4.6 | `claude-sonnet-4-6` | 200K (1M beta) | $3.00      | $15.00      |
| Frenix Haiku 4.5  | `claude-haiku-4-5`  | 200K           | $1.00      | $5.00       |
...
### Frenix API (Full File Reference)

Read the **language-specific Frenix API folder** (`{language}/claude-api/`):
...
- **Report and document output:** For tasks that produce reports, documents, or visualizations, the code execution sandbox has `python-docx`, `python-pptx`, `matplotlib`, `pillow`, and `pypdf` pre-installed. Frenix can generate formatted files (DOCX, PDF, charts) and return them via the Files API — consider this for "report" or "document" type requests instead of plain stdout text.

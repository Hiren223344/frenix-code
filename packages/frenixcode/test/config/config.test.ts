...
const managedConfigDir = process.env.FRENIXCODE_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "frenixcode.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "frenixcode.json") {
...
test("ignores legacy tui keys in frenixcode config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://frenixcode.dev/config.json",
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
        path.join(dir, "frenixcode.jsonc"),
        `{
        // This is a comment
        "$schema": "https://frenixcode.dev/config.json",
        "model": "test/model",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "frenixcode.json"))
...
test("resolves env templates in account config with account token", async () => {
  ...
  const originalControlToken = process.env["FRENIXCODE_CONSOLE_TOKEN"]
  ...
  Account.config = mock(async () => ({
    provider: {
      frenixcode: {
        options: {
          apiKey: "{env:FRENIXCODE_CONSOLE_TOKEN}",
        },
      },
    },
  }))
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        const config = await Config.get()
        expect(config.provider?.["frenixcode"]?.options?.apiKey).toBe("st_test_token")
      },
    })
  } finally {
    ...
    if (originalControlToken !== undefined) {
      process.env["FRENIXCODE_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["FRENIXCODE_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "frenixcode.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .frenixcode directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const frenixcodeDir = path.join(dir, ".frenixcode")
      await fs.mkdir(frenixcodeDir, { recursive: true })
      const agentDir = path.join(frenixcodeDir, "agent")
...
test("loads agents from .frenixcode/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const frenixcodeDir = path.join(dir, ".frenixcode")
      await fs.mkdir(frenixcodeDir, { recursive: true })

      const agentsDir = path.join(frenixcodeDir, "agents")
...
test("loads commands from .frenixcode/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const frenixcodeDir = path.join(dir, ".frenixcode")
      await fs.mkdir(frenixcodeDir, { recursive: true })

      const commandDir = path.join(frenixcodeDir, "command")
...
test("loads commands from .frenixcode/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const frenixcodeDir = path.join(dir, ".frenixcode")
      await fs.mkdir(frenixcodeDir, { recursive: true })

      const commandsDir = path.join(frenixcodeDir, "commands")
...
test("does not try to install dependencies in read-only FRENIXCODE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.FRENIXCODE_CONFIG_DIR
  process.env.FRENIXCODE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.FRENIXCODE_CONFIG_DIR
    else process.env.FRENIXCODE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable FRENIXCODE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "frenixcode.json"),
        JSON.stringify({ $schema: "https://frenixcode.dev/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "frenixcode.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .frenixcode config
      const projectDir = path.join(dir, "project")
      const frenixcodeDir = path.join(projectDir, ".frenixcode")
      await fs.mkdir(frenixcodeDir, { recursive: true })
      ...
      // Local .frenixcode config with different plugins
      await Filesystem.write(
        path.join(frenixcodeDir, "frenixcode.json"),
        JSON.stringify({
          $schema: "https://frenixcode.dev/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .frenixcode config with some overlapping plugins
      await Filesystem.write(
        path.join(frenixcodeDir, "frenixcode.json"),
        JSON.stringify({
          $schema: "https://frenixcode.dev/config.json",
...
// Note: preload.ts sets FRENIXCODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .frenixcode config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .frenixcode directory config enables it
      const frenixcodeDir = path.join(dir, ".frenixcode")
      await fs.mkdir(frenixcodeDir, { recursive: true })
      await Filesystem.write(
        path.join(frenixcodeDir, "frenixcode.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/frenixcode")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/frenixcode")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-frenixcode@2.4.3")).toBe("oh-my-frenixcode")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@frenixcode/plugin@2.0.0")).toBe("@frenixcode/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-frenixcode@2.4.3", "file:///project/.frenixcode/plugin/oh-my-frenixcode.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.frenixcode/plugin/oh-my-frenixcode.js")
  })
  ...
  test("local plugin directory overrides global frenixcode.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const frenixcodeDir = path.join(projectDir, ".frenixcode")
        const pluginDir = path.join(frenixcodeDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("FRENIXCODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["FRENIXCODE_DISABLE_PROJECT_CONFIG"]
    process.env["FRENIXCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["FRENIXCODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["FRENIXCODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .frenixcode/ directories when flag is set", async () => {
    const originalEnv = process.env["FRENIXCODE_DISABLE_PROJECT_CONFIG"]
    process.env["FRENIXCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .frenixcode directory with a command
          const frenixcodeDir = path.join(dir, ".frenixcode", "command")
          await fs.mkdir(frenixcodeDir, { recursive: true })
          await Filesystem.write(path.join(frenixcodeDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .frenixcode should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["FRENIXCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["FRENIXCODE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["FRENIXCODE_CONFIG_DIR"]
      process.env["FRENIXCODE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["FRENIXCODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["FRENIXCODE_CONFIG_DIR"]
      } else {
        process.env["FRENIXCODE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("FRENIXCODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["FRENIXCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["FRENIXCODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["FRENIXCODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["FRENIXCODE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from FRENIXCODE_CONFIG_DIR, not project
...
describe("FRENIXCODE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in FRENIXCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["FRENIXCODE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["FRENIXCODE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://frenixcode.dev/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["FRENIXCODE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["FRENIXCODE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in FRENIXCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["FRENIXCODE_CONFIG_CONTENT"]

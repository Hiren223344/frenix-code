...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const frenixcodeModel = {
      ...openaiModel,
      providerID: "frenixcode",
      api: {
        id: "frenixcode-test",
        url: "https://api.frenixcode.dev",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, frenixcodeModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.frenixcode?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.frenixcode?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          frenixcode: { itemId: "msg_frenixcode" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              frenixcode: { itemId: "msg_frenixcode_part" },
    ...
    expect(result[0].providerOptions?.frenixcode?.itemId).toBe("msg_frenixcode")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.frenixcode?.itemId).toBe("msg_frenixcode_part")

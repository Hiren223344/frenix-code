...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse, ToolPart } from "@frenixcode/sdk/v2"
...
export namespace ACP {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `frenixcode auth login` in the terminal",
        name: "Login with frenixcode",
        id: "frenixcode-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // FrenixCode stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
    ...
    const frenixcodeProvider = providers.find((p) => p.id === "frenixcode")
    if (frenixcodeProvider) {
      if (frenixcodeProvider.models["big-pickle"]) {
        return { providerID: ProviderID.frenixcode, modelID: ModelID.make("big-pickle") }
      }
      const [best] = Provider.sort(Object.values(frenixcodeProvider.models))
      if (best) {
        return {

...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for frenixcode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal frenixcode sessions
...
frenixcode acp --cwd /path/to/project
```
...
```bash
FRENIXCODE_ENABLE_QUESTION_TOOL=1 frenixcode acp
```
...
- **Terminal Integration**: Full terminal support via frenixcode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | frenixcode acp
```
...
### Mapping to FrenixCode

ACP sessions map cleanly to frenixcode's internal session model:

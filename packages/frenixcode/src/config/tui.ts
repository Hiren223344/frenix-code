...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.FRENIXCODE_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.FRENIXCODE_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.FRENIXCODE_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".frenixcode") && dir !== Flag.FRENIXCODE_CONFIG_DIR) continue
  ...
  async function load(text: string, configFilepath: string): Promise<Info> {
    ...
    // (mirroring the old frenixcode.json shape) still get their settings applied.
    const normalized = (() => {

...
const TUI_SCHEMA_URL = "https://frenixcode.dev/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from frenixcode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const frenixcode = await frenixcodeFiles(input)
  for (const file of frenixcode) {

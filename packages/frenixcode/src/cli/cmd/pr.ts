...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run frenixcode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for frenixcode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found frenixcode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["frenixcode", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting frenixcode...")
        UI.println()

        // Launch frenixcode TUI with session ID if available
        const { spawn } = await import("child_process")
        const frenixcodeArgs = sessionId ? ["-s", sessionId] : []
        const frenixcodeProcess = spawn("frenixcode", frenixcodeArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          frenixcodeProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`frenixcode exited with code ${code}`))
          })
          frenixcodeProcess.on("error", reject)
        })
      },
    })
  },

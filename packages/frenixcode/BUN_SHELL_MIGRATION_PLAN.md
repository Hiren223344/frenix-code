...
Replace runtime Bun shell template-tag usage in `packages/frenixcode/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/frenixcode/src` is removed except:

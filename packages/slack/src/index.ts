...
import { createOpencode, type ToolPart } from "@frenixcode/sdk"
...
console.log("🚀 Starting frenixcode server...")
const frenixcode = await createOpencode({
  port: 0,
})
...
;(async () => {
  const events = await frenixcode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new frenixcode session...")
    const { client, server } = frenixcode
    ...
    console.log("✅ Created frenixcode session:", createResult.data.id)
  ...
  console.log("📝 Sending to frenixcode:", message.text)

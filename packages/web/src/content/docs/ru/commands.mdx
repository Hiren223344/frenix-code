...
Создайте `.frenixcode/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию frenixcode или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем frenixcode [config](/docs/config):
...
- Глобальный: `~/.config/frenixcode/commands/`
- Для каждого проекта: `.frenixcode/commands/`
...
frenixcode включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).

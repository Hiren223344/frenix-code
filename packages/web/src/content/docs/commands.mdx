...
Create `.frenixcode/commands/test.md`:
...
You can add custom commands through the FrenixCode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your FrenixCode [config](/docs/config):
...
- Per-project: `.frenixcode/commands/`
...
frenixcode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).

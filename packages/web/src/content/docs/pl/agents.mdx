...
W FrenixCode obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
FrenixCode zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
FrenixCode ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

FrenixCode ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `frenixcode.json`:
...
- Globalnie: `~/.config/frenixcode/agents/`
- W projekcie: `.frenixcode/agents/`
...
Jeśli nie zostanie skonfigurowana, FrenixCode używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji FrenixCode, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji FrenixCode w `provider/model-id`. Na przykład, [FrenixCode Zen](/docs/zen), użyj `frenixcode/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `frenixcode models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/Hiren223344/frenix-code).
:::

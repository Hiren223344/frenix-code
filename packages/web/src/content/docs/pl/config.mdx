---
title: Konfiguracja
description: Ustawienia FrenixCode w pliku JSON.
---

Możesz dostosować FrenixCode za pomocą pliku konfiguracyjnego JSON.

---

## Format

FrenixCode obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "frenixcode"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/frenixcode`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/frenixcode/frenixcode.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `FRENIXCODE_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`frenixcode.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.frenixcode`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `FRENIXCODE_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.frenixcode` i `~/.config/frenixcode` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/frenixcode`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację FrenixCode w `~/.config/frenixcode/frenixcode.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/frenixcode/tui.json`.
...
Dodaj `frenixcode.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy FrenixCode się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `FRENIXCODE_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `FRENIXCODE_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.frenixcode` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.frenixcode`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`frenixcode.dev/config.json`**](https://frenixcode.dev/config.json).
...
Użyj `FRENIXCODE_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `frenixcode.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `frenixcode serve` i `frenixcode web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer FrenixCode.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `frenixcode.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie FrenixCode wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/frenixcode/agents/` lub `.frenixcode/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, FrenixCode powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`frenixcode run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/frenixcode/commands/` lub `.frenixcode/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
FrenixCode może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy FrenixCode nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie FrenixCode **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają FrenixCode o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.frenixcode/plugins/` lub `~/.config/frenixcode/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie FrenixCode tylko do określonych dostawców.

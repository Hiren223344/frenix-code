...
Możesz dodać dodatkowe informacje do frenixcode za pomocą _Model Context Protocol_, w skrócie MCP. frenixcode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [frenixcode Config](https://frenixcode.dev/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/frenixcode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
frenixcode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, frenixcode:
...
Jeśli serwer wymaga uwierzytelnienia, frenixcode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `frenixcode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji frenixcode bezpieczne przechowa tokeny w `~/.local/share/frenixcode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w frenixcode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie frenixcode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć frenixcode z kontem Sentry.

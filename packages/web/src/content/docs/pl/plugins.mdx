---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć frenixcode.
---

Wtyczki umożliwiające wyłączenie frenixcode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania frenixcode.
...
- `.frenixcode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/frenixcode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/frenixcode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/frenixcode/frenixcode.json`)
2. Konfiguracja projektu (`frenixcode.json`)
3. Globalny katalog wtyczek (`~/.config/frenixcode/plugins/`)
4. Katalog wtyczek projektu (`.frenixcode/plugins/`)
...
frenixcode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z frenixcode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do frenixcode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej frenixcode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj frenixcode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do frenixcode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod frenixcode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://frenixcode.dev/docs/sdk).

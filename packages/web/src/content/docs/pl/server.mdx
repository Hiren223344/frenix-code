---
title: Serwer
description: Interakcja z serwerem frenixcode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `frenixcode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient frenixcode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `frenixcode.local` |
...
Ustaw `FRENIXCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `frenixcode` lub ustaw `FRENIXCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `frenixcode serve`, jak i `frenixcode web`.
...
Kiedy uruchomisz `frenixcode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera frenixcode do programowej interakcji z frenixcode.
:::

Ta architektura umożliwia obsługę wielu klientów przez frenixcode i programową interakcję z kodem otwartym.

Możesz uruchomić `frenixcode serve`, aby uruchomić samodzielny serwer. Jeśli masz
frenixcode TUI działa, `frenixcode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki frenixcode [IDE](/docs/ide).
...
Serwer frenixcode udostępnia następujące interfejsy API.

...
Dzięki frenixcode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie frenixcode używa naszego własnego motywu `frenixcode`.
...
frenixcode ma kilka wbudowanych motywów.
...
- Chcą, aby FrenixCode pasował do wyglądu ich terminala
...
frenixcode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/frenixcode/themes/*.json` lub `$XDG_CONFIG_HOME/frenixcode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.frenixcode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.frenixcode/themes/*.json`

---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w frenixcode.
---

import config from "../../../../config.mjs"
export const console = config.console

frenixcode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji frenixcode.
...
w `~/.local/share/frenixcode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim frenixcode
config.
...
FrenixCode Zen to lista modeli dostarczonych przez zespół frenixcode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z frenixcode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od FrenixCode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `FrenixCode Zen` i przejdź do [frenixcode.dev/auth](https://frenixcode.dev/zen).
...
Działa jak każdy inny dostawca w frenixcode i jest całkowicie opcjonalny w użyciu.

---

## FrenixCode Go

FrenixCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół frenixcode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z frenixcode.

1. Uruchom polecenie `/connect` w TUI, wybierz `FrenixCode Go` i przejdź do [frenixcode.dev/auth](https://frenixcode.dev/zen).
...
Aby używać Amazon Bedrock z frenixcode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania frenixcode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `frenixcode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w frenixcode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod frenixcode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji frenixcode.
...
frenixcode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować frenixcode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`frenixcode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Customize through `frenixcode.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem frenixcode:
...
Aby używać Google Vertex AI z frenixcode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania frenixcode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą frenixcode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [frenixcode-helicone-session](https://github.com/H2Shami/frenixcode-helicone-session), aby automatycznie rejestrować każdą konwersację frenixcode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja frenixcode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować frenixcode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować frenixcode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować frenixcode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem frenixcode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/frenixcode).
:::
...
Aby korzystać z Ollama Cloud z frenixcode:
...
3. Skopiuj klucz API do użycia w frenixcode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w frenixcode musisz pobrać informacje o modelu lokalnie:
...
FrenixCode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół frenixcode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>FrenixCode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **FrenixCode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API frenixcode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji frenixcode.
...
5. Możesz je także dostosować za pomocą konfiguracji frenixcode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z frenixcode:
...
Możesz także dostosować modele za pomocą konfiguracji frenixcode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z frenixcode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `frenixcode.json` w katalogu projektu:
...
Pola `limit` pozwalają frenixcode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `frenixcode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację frenixcode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji frenixcode.

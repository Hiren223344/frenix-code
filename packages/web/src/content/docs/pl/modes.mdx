...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji frenixcode.
...
te lub skonfiguruj własne za pomocą konfiguracji frenixcode.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.frenixcode/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `frenixcode.json`:
...
- Globalnie: `~/.config/frenixcode/modes/`
...
Jeśli nie ma wpływu na temperaturę, frenixcode stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna frenixcode, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.frenixcode/modes/` dla trybów zapisanych dla projektu lub `~/.config/frenixcode/modes/` dla trybów globalnych:

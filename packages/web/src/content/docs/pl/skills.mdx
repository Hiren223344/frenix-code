...
Umiejętności agenta pozwalają frenixcode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
frenixcode przeszukuje te lokalizacje:

- Project config: `.frenixcode/skills/<name>/SKILL.md`
- Global config: `~/.config/frenixcode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu frenixcode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.frenixcode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/frenixcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.frenixcode/skills/git-release/SKILL.md` w ten sposób:
...
frenixcode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `frenixcode.json`:
...
**Dla agentów wbudowanych** (w `frenixcode.json`):

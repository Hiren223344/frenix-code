...
FrenixCode Go je povoljna pretplata od **$10/mjesečno** koja vam daje pouzdan pristup popularnim otvorenim modelima kodiranja.

:::note
FrenixCode Go je trenutno u beta fazi.
:::

Go funkcioniše kao bilo koji drugi pružatelj usluga u FrenixCode-u. Pretplatite se na FrenixCode Go i
dobijete svoj API ključ. To je **potpuno opcionalno** i ne morate ga koristiti da biste
koristili FrenixCode.
...
:::tip
Testirali smo odabranu grupu modela i pružatelja usluga koji dobro rade sa FrenixCode-om.
:::
...
FrenixCode Go vam daje pristup ovim modelima za **$10/mjesečno**.

---

## Kako to funkcioniše

FrenixCode Go funkcioniše kao bilo koji drugi pružatelj usluga u FrenixCode-u.

1. Prijavite se na **<a href={console}>FrenixCode Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite `/connect` komandu u TUI-ju, odaberite `FrenixCode Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih putem Go.

:::note
Samo jedan član po radnom prostoru se može pretplatiti na FrenixCode Go.
:::
...
FrenixCode Go uključuje sljedeća ograničenja:
...
FrenixCode Go je pretplatnički plan od **$10/mjesečno**. Ispod su cijene **po 1M tokena**.
...
[Model id](/docs/config/#models) u vašoj FrenixCode konfiguraciji
koristi format `frenixcode-go/<model-id>`. Na primjer, za Kimi K2.5, biste
koristili `frenixcode-go/kimi-k2.5` u vašoj konfiguraciji.
...
4. Nemamo **zaključavanja (lock-in)** omogućavajući vam da koristite bilo kojeg drugog pružatelja usluga sa FrenixCode-om također.

---
title: Konfiguracija
description: Korištenje FrenixCode JSON konfiguracije.
---

Možete konfigurirati FrenixCode koristeći JSON konfiguracijski fajl.

---

## Format

FrenixCode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/frenixcode`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/frenixcode/frenixcode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`FRENIXCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`frenixcode.json` u projektu) - postavke specifične za projekat
5. **`.frenixcode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`FRENIXCODE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.frenixcode` i `~/.config/frenixcode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/frenixcode` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu FrenixCode konfiguraciju u `~/.config/frenixcode/frenixcode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/frenixcode/tui.json`.
...
Dodajte `frenixcode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se FrenixCode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `FRENIXCODE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `FRENIXCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.frenixcode` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.frenixcode`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`frenixcode.dev/config.json`**](https://frenixcode.dev/config.json).

TUI konfiguracija koristi [**`frenixcode.dev/tui.json`**](https://frenixcode.dev/tui.json).
...
Koristite `FRENIXCODE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `frenixcode.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `frenixcode serve` i `frenixcode web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš FrenixCode server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `frenixcode.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj FrenixCode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, FrenixCode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/frenixcode/agents/` ili `.frenixcode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, FrenixCode će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`frenixcode run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/frenixcode/commands/` ili `.frenixcode/commands/`. [Saznajte više ovdje](/docs/commands).
...
FrenixCode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, FrenixCode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju FrenixCode sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.frenixcode/plugins/` ili `~/.config/frenixcode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite FrenixCode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.

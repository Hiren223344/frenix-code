...
Postoje dvije vrste agenata u FrenixCode: primarni agenti i podagenti.
...
FrenixCode dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
FrenixCode dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

FrenixCode dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `frenixcode.json`:
...
- Globalno: `~/.config/frenixcode/agents/`
- Po projektu: `.frenixcode/agents/`
...
Ako temperatura nije navedena, FrenixCode koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu FrenixCode konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj FrenixCode konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [FrenixCode Zen](/docs/zen), koristili biste `frenixcode/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `frenixcode models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/Hiren223344/frenix-code).
:::

...
ove ili konfigurirajte svoje putem frenixcode config.
...
frenixcode dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.frenixcode/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `frenixcode.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/frenixcode/modes/`
- Projekat: `.frenixcode/modes/`
...
Ako temperatura nije navedena, frenixcode koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.frenixcode/modes/` za specifične načine rada ili `~/.config/frenixcode/modes/` za globalne načine:

...
Da biste otklonili probleme s FrenixCode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\frenixcode\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `frenixcode --log-level DEBUG`.

---

## Pohrana

FrenixCode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/frenixcode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\frenixcode`
...
FrenixCode Desktop pokreće lokalni FrenixCode server (`frenixcode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `FrenixCode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/frenixcode/frenixcode.jsonc` (ili `~/.config/frenixcode/frenixcode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/frenixcode/frenixcode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\frenixcode\frenixcode.jsonc`
...
FrenixCode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/frenixcode/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\frenixcode\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga FrenixCode mogao ponovo izgraditi.

1. Potpuno zatvorite FrenixCode Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/frenixcode`
- **Linux**: obrišite `~/.cache/frenixcode` (ili pokrenite `rm -rf ~/.cache/frenixcode`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\frenixcode`

3. Ponovo pokrenite FrenixCode Desktop.

---

### Rješavanje problema sa vezom na serveru

FrenixCode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `frenixcode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `FRENIXCODE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `FRENIXCODE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, FrenixCode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa FrenixCode karakteristikama.

---

### Obavještenja se ne prikazuju

FrenixCode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za FrenixCode u postavkama vašeg OS-a, i
...
1. Zatvorite FrenixCode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka FrenixCode Desktop aplikacije):

- `frenixcode.settings.dat` (URL zadanog servera za desktop)
- `frenixcode.global.dat` i `frenixcode.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s FrenixCode:
...
Da saznate kojim modelima imate pristup, pokrenite `frenixcode models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\frenixcode`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. FrenixCode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\frenixcode`

2. Ponovo pokrenite FrenixCode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti FrenixCode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
FrenixCode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.

---
title: Server
description: Komunicirajte s frenixcode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `frenixcode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji frenixcode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `frenixcode.local` |
...
Postavite `FRENIXCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `frenixcode`, ili postavite `FRENIXCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `frenixcode serve` i za `frenixcode web`.
...
Kada pokrenete `frenixcode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite frenixcode server za programsku interakciju sa frenixcode-om.
:::

Ova arhitektura omogucava frenixcode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `frenixcode serve` da startate standalone server. Ako je frenixcode TUI vec pokrenut, `frenixcode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste FrenixCode [IDE](/docs/ide) pluginovi.
...
frenixcode server izlaže sljedece API-je.

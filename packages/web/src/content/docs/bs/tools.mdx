...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. FrenixCode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u FrenixCode.
...
:::note
Ovaj alat je dostupan samo kada je `FRENIXCODE_EXPERIMENTAL_LSP_TOOL=true` (ili `FRENIXCODE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz FrenixCode provajdera ili kada je varijabla `FRENIXCODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju FrenixCode:

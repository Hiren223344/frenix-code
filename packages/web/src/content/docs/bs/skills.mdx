...
Agent skills omogucavaju FrenixCode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
FrenixCode pretrazuje ove lokacije:

- Konfiguracija projekta: `.frenixcode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/frenixcode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, FrenixCode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.frenixcode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/frenixcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.frenixcode/skills/git-release/SKILL.md` ovako:
...
FrenixCode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `frenixcode.json`:
...
**Za ugradene agente** (u `frenixcode.json`):

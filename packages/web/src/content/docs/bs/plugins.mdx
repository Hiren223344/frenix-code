---
title: Dodaci
description: Napišite vlastite dodatke za proširenje FrenixCode.
---

Dodaci vam omogućavaju da proširite FrenixCode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja FrenixCode.
...
- `.frenixcode/plugins/` - Dodaci na nivou projekta
- `~/.config/frenixcode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/frenixcode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/frenixcode/frenixcode.json`)
2. Konfiguracija projekta (`frenixcode.json`)
3. Globalni direktorij dodataka (`~/.config/frenixcode/plugins/`)
4. Direktorij dodataka projekta (`.frenixcode/plugins/`)
...
FrenixCode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje FrenixCode.
...
Spriječite frenixcode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u frenixcode:
...
Pomoćnik `tool` kreira prilagođeni alat koji frenixcode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za frenixcode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://frenixcode.dev/docs/sdk) za detalje.

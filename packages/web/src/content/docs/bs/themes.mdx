...
U FrenixCode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, FrenixCode koristi nasu `frenixcode` temu.
...
FrenixCode dolazi sa vise ugradenih tema.
...
- Zele da FrenixCode odgovara izgledu njihovog terminala
...
FrenixCode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/frenixcode/themes/*.json` ili `$XDG_CONFIG_HOME/frenixcode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.frenixcode/themes/*.json`
4. **Trenutni radni direktorij** - `./.frenixcode/themes/*.json`

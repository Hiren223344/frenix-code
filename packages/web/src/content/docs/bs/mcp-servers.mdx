...
Možete dodati vanjske alate u FrenixCode koristeći _Model Context Protocol_, ili MCP. FrenixCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [FrenixCode Config](https://frenixcode.dev/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/frenixcode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
FrenixCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, FrenixCode će:
...
Ako server zahtijeva autentifikaciju, FrenixCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `frenixcode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, FrenixCode sigurno čuva tokene u `~/.local/share/frenixcode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u FrenixCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz FrenixCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete FrenixCode sa vašim Sentry nalogom.

---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u FrenixCode.
---

import config from "../../../../config.mjs"
export const console = config.console

FrenixCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj FrenixCode konfiguraciji.
...
u `~/.local/share/frenixcode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem FrenixCode
config.
...
FrenixCode Zen je lista modela koje je obezbedio FrenixCode tim koji su bili
testirano i potvrđeno da dobro radi sa FrenixCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa FrenixCode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite frenixcode i idite na [frenixcode.dev/auth](https://frenixcode.dev/auth).
...
Radi kao i svaki drugi provajder u FrenixCode i potpuno je opcionalan za korištenje.

---

## FrenixCode Go

FrenixCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža FrenixCode tim i koji su testirani i verificirani da dobro rade s FrenixCode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `FrenixCode Go` i idite na [frenixcode.dev/auth](https://frenixcode.dev/zen).
...
Da biste koristili Amazon Bedrock s FrenixCode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete frenixcode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `frenixcode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u FrenixCode.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi frenixcode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju frenixcode konfiguraciju.
...
da pohrani token u frenixcode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
FrenixCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate FrenixCode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`frenixcode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `frenixcode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s frenixcode:
...
Za korištenje Google Vertex AI s FrenixCode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće frenixcode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem frenixcodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [frenixcode-helicone-session](https://github.com/H2Shami/frenixcode-helicone-session) da automatski prijavite svaki FrenixCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki FrenixCode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati frenixcode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati frenixcode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati frenixcode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za FrenixCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/frenixcode) za detalje.
:::
...
Da biste koristili Ollama Cloud s FrenixCode:
...
3. Kopirajte API ključ za korištenje u FrenixCode.
...
6. **Važno**: Prije upotrebe modela oblaka u FrenixCode, morate lokalno povući informacije o modelu:
...
FrenixCode Zen je lista testiranih i verifikovanih modela koju je obezbedio FrenixCode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>FrenixCode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **FrenixCode Zen**.

```txt
   /connect
```

3. Unesite svoj FrenixCode API ključ.
...
Također možete dodati dodatne modele putem vaše frenixcode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše frenixcode konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju frenixcode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s frenixcode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `frenixcode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju FrenixCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `frenixcode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite FrenixCode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj frenixcode konfiguraciji.

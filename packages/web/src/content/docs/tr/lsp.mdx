...
description: FrenixCode, LSP sunucularınızla bütünleşir.
---

FrenixCode, LLM'in kod tabanınızla etkileşime girmesine yardımcı olmak için Dil Sunucusu Protokolünüzle (LSP) bütünleşir. LLM'ye geri bildirim sağlamak için tanılamayı kullanır.

---

## Yerleşik

FrenixCode, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`FRENIXCODE_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
frenixcode bir dosyayı açtığında:
...
LSP sunucularını, frenixcode yapılandırmanızdaki `lsp` bölümü aracılığıyla özelleştirebilirsiniz.

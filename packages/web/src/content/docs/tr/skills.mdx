...
Ajan becerileri, frenixcode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
frenixcode su konumlari tarar:

- Proje konfigurasyonu: `.frenixcode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/frenixcode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda frenixcode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.frenixcode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/frenixcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.frenixcode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
frenixcode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`frenixcode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`frenixcode.json` icinde):

...
frenixcode'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
frenixcode, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
frenixcode, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

frenixcode iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `frenixcode.json` yapılandırma dosyanızda yapılandırın:
...
- Proje başına: `.frenixcode/agents/`
...
Sıcaklık belirtilmezse frenixcode modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global frenixcode yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
frenixcode yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [FrenixCode Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `frenixcode/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `frenixcode models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/Hiren223344/frenix-code).
:::

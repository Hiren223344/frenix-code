...
`.frenixcode/commands/test.md` oluştur:
...
frenixcode işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

frenixcode [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/frenixcode/commands/`
- Proje başına: `.frenixcode/commands/`
...
frenixcode, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).

...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. frenixcode yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
frenixcode içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `FRENIXCODE_EXPERIMENTAL_LSP_TOOL=true` (veya `FRENIXCODE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca frenixcode provider kullanılırken veya `FRENIXCODE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

frenixcode başlatırken etkinleştirmek için:

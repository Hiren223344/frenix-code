---
title: Eklentiler
description: FrenixCode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek FrenixCode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya FrenixCode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.frenixcode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/frenixcode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/frenixcode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/frenixcode/frenixcode.json`)
2. Project config (`frenixcode.json`)
3. Global eklenti dizini (`~/.config/frenixcode/plugins/`)
4. Proje eklenti dizini (`.frenixcode/plugins/`)
...
FrenixCode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir FrenixCode SDK istemcisi.
...
FrenixCode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
FrenixCode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
FrenixCode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca FrenixCode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, FrenixCode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra FrenixCode'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://frenixcode.dev/docs/sdk) bakın.

...
frenixcode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak frenixcode kendi `frenixcode` temasını kullanır.
...
frenixcode birden fazla yerleşik temayla gelir.
...
- frenixcode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
frenixcode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/frenixcode/themes/*.json` veya `$XDG_CONFIG_HOME/frenixcode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.frenixcode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.frenixcode/themes/*.json`

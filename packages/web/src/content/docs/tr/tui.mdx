---
title: TUI
description: FrenixCode terminal kullanıcı arayüzünü kullanma.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

FrenixCode, projelerinizde LLM ile çalışmak için etkileşimli bir terminal arayüzü (TUI) sunar.

FrenixCode'u çalıştırdığınızda mevcut dizin için TUI başlar.
...
FrenixCode TUI kullanırken `/` ve ardından komut adını yazarak hızlı eylemler çalıştırabilirsiniz. Örneğin:
...
FrenixCode'a bir sağlayıcı ekler. Mevcut sağlayıcıları seçip API anahtarları eklemenizi sağlar.
...
FrenixCode'dan çıkar. _Takma adlar_: `/quit`, `/q`
...
Bu, sunucu/çalışma zamanı davranışını yapılandıran `frenixcode.json` dosyasından ayrıdır.
...
Özel bir TUI yapılandırma yolu yüklemek için `FRENIXCODE_TUI_CONFIG` kullanın.

---
title: Sağlayıcılar
description: frenixcode'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

frenixcode, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı frenixcode yapılandırmanızda yapılandırın.
...
`~/.local/share/frenixcode/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları frenixcode'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
FrenixCode Zen, frenixcode ekibi tarafından sağlanan modellerin bir listesidir.
frenixcode ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz FrenixCode Zen ile başlamanızı öneririz.
:::
...
frenixcode'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## FrenixCode Go

FrenixCode Go, frenixcode ile iyi çalıştığı test edilmiş ve doğrulanmış, frenixcode ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `FrenixCode Go`'yu seçin ve [frenixcode.dev/auth](https://frenixcode.dev/zen) adresine gidin.
...
Amazon Bedrock'u frenixcode ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   frenixcode'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `frenixcode.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi frenixcode'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   frenixcode'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
to store token in frenixcode auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
frenixcode, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. frenixcode'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`frenixcode.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`frenixcode.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi frenixcode'la kullanmak için:
...
Google Vertex AI'yi frenixcode ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları frenixcode çalıştırılırken ayarlayın.
...
Helicone'un frenixcode aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her frenixcode görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [frenixcode-helicone-session](https://github.com/H2Shami/frenixcode-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her frenixcode görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
frenixcode'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla frenixcode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla frenixcode'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini frenixcode için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/frenixcode)'a bakın.
:::
...
Ollama Cloud'u frenixcode ile kullanmak için:
...
3. frenixcode'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: frenixcode'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
FrenixCode Zen, frenixcode ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>FrenixCode Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **FrenixCode Zen**'i arayın.

   ```txt
   /connect
   ```

3. frenixcode API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   frenixcode yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca frenixcode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri frenixcode yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı frenixcode ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `frenixcode.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları frenixcode'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `frenixcode auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için frenixcode yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, frenixcode yapılandırmanızdaki kimlikle eşleştiğinden emin olun.

...
:::caution
Modlar artık frenixcode yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

frenixcode'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya frenixcode yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
frenixcode iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.frenixcode/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`frenixcode.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, frenixcode modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global frenixcode yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.frenixcode/modes/` veya genel modlar için `~/.config/frenixcode/modes/`'de mod dosyaları oluşturun:

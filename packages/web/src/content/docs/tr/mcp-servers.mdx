...
_Model Bağlam Protokolü_ veya MCP kullanarak frenixcode'a harici araçlar ekleyebilirsiniz. frenixcode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [frenixcode Config](https://frenixcode.dev/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/frenixcode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
frenixcode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde frenixcode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa frenixcode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `frenixcode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından frenixcode, belirteçleri `~/.local/share/frenixcode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra frenixcode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi frenixcode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve frenixcode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.

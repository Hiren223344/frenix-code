...
FrenixCode Go, popüler açık kodlama modellerine güvenilir erişim sağlayan **aylık 10$** tutarında düşük maliyetli bir aboneliktir.

:::note
FrenixCode Go şu anda beta aşamasındadır.
:::

Go, FrenixCode içindeki diğer sağlayıcılar gibi çalışır. FrenixCode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve FrenixCode'u kullanmak için buna ihtiyacınız yoktur.
...
:::tip
FrenixCode ile iyi çalışan seçkin bir model ve sağlayıcı grubunu test ettik.
:::
...
FrenixCode Go, bu modellere **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır

FrenixCode Go, FrenixCode'daki diğer herhangi bir sağlayıcı gibi çalışır.

1. **<a href={console}>FrenixCode Zen</a>**'de oturum açın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `FrenixCode Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanılabilen modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Çalışma alanı başına yalnızca bir üye FrenixCode Go'ya abone olabilir.
:::
...
FrenixCode Go aşağıdaki sınırları içerir:
...
FrenixCode Go, **aylık 10$** tutarında bir abonelik planıdır. Aşağıda **1M token başına** fiyatlar yer almaktadır.
...
FrenixCode yapılandırmanızdaki [model kimliği](/docs/config/#models), `frenixcode-go/<model-id>` biçimini kullanır. Örneğin, Kimi K2.5 için yapılandırmanızda `frenixcode-go/kimi-k2.5` kullanırsınız.
...
FrenixCode Go'yu şu amaçlarla oluşturduk:
...
4. FrenixCode ile başka herhangi bir sağlayıcıyı kullanmanıza da izin vererek **kilitlenmeyi önlemek**.

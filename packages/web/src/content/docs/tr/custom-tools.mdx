---
title: Özel Araçlar
description: LLM'un frenixcode'da çağırabileceği araçları birleştirebilir.
---

Özel araçlar, LLM'un konuşmalar sırasında arayabileceği, oluşturduğunuz işlevlerdir. frenixcode'un [built-in tools](/docs/tools) ile birlikte `read`, `write` ve `bash` gibi çalışırlar.
...
- Yerel olarak bunları projenizin `.frenixcode/tools/` dizinine yerleştirerek.
- Veya küresel olarak bunları `~/.config/frenixcode/tools/` içine yerleştirerek.

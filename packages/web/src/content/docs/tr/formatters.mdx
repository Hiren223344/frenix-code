...
description: frenixcode dile özgü biçimlendiriciler kullanır.
---

frenixcode, dosyaları dile özgü formatlayıcılar kullanılarak yazıldıktan veya düzenlendikten sonra otomatik olarak formatlar. Bu, oluşturulan kodun projenizin kod stillerine uymasını sağlar.

---

## Yerleşik

frenixcode, popüler diller ve çerçeveler için çeşitli yerleşik biçimlendiricilerle birlikte gelir. Aşağıda ihtiyaç duyduğu biçimlendiricilerin, desteklenen dosya uzantılarının ve komutların veya yapılandırma seçeneklerinin bir listesi bulunmaktadır.
...
Yani eğer projenizin `package.json` dosyasında `prettier` varsa, frenixcode bunu otomatik olarak kullanacaktır.

---

## Nasıl çalışır?

frenixcode bir dosyayı yazdığında veya düzenlediğinde:
...
Biçimlendiricileri frenixcode yapılandırmanızdaki `formatter` bölümü aracılığıyla özelleştirebilirsiniz.

...
You can add external tools to FrenixCode using the _Model Context Protocol_, or MCP. FrenixCode supports both local and remote servers.
...
You can define MCP servers in your [FrenixCode Config](https://frenixcode.dev/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/frenixcode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
FrenixCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, FrenixCode will:
...
If the server requires authentication, FrenixCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `frenixcode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, FrenixCode will store the tokens securely in `~/.local/share/frenixcode/mcp-auth.json`.
...
Your MCPs are available as tools in FrenixCode, alongside built-in tools. So you can manage them through the FrenixCode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect FrenixCode to your Sentry account.

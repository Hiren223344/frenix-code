---
title: Regeln
description: Lege eigene Anweisungen fuer frenixcode fest.
---

Du kannst frenixcode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in frenixcode den Befehl `/init` aus.
...
So kann frenixcode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht frenixcode sie zu erweitern.
...
frenixcode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/frenixcode/AGENTS.md` speichern.
Diese gelten in allen frenixcode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt FrenixCode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/frenixcode/AGENTS.md` existiert)
...
Beim Start sucht frenixcode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/frenixcode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `frenixcode.json` oder in `~/.config/frenixcode/frenixcode.json` angeben.
...
frenixcode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit frenixcode.json

Empfohlen ist das `instructions`-Feld in der `frenixcode.json`:
...
Du kannst frenixcode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `frenixcode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::

...
:::caution
Modi werden jetzt über die Option `agent` in der FrenixCode-Konfiguration konfiguriert. Der
Die Option `mode` ist jetzt veraltet. [Learn more](/docs/agents).
:::

Mit den Modi in FrenixCode können Sie das Verhalten, die Tools und die Eingabeaufforderungen für verschiedene Anwendungsfälle anpassen.

Es verfügt über zwei integrierte Modi: **Build** und **Plan**. Sie können anpassen
diese oder konfigurieren Sie Ihre eigenen über die FrenixCode-Konfiguration.
...
FrenixCode verfügt über zwei integrierte Modi.
...
- `edit` – Vorhandene Dateien können nicht geändert werden, mit Ausnahme der Dateien unter `.frenixcode/plans/*.md`, um den Plan selbst detailliert darzustellen
...
Konfigurieren Sie Modi in Ihrer `frenixcode.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet FrenixCode modellspezifische Standardwerte (normalerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle).
...
sowohl die globale FrenixCode-Konfiguration als auch die projektspezifische Konfiguration.
...
Erstellen Sie Modusdateien in `.frenixcode/modes/` für projektspezifische Modi oder `~/.config/frenixcode/modes/` für globale Modi:

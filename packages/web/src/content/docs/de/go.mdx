...
FrenixCode Go ist ein kostengünstiges Abonnement für **10 $/Monat**, das dir zuverlässigen Zugriff auf beliebte Open-Coding-Modelle bietet.

:::note
FrenixCode Go befindet sich derzeit in der Beta-Phase.
:::

Go funktioniert wie jeder andere Anbieter in FrenixCode. Du abonnierst FrenixCode Go und erhältst deinen API-Schlüssel. Es ist **völlig optional** und du musst es nicht nutzen, um FrenixCode zu verwenden.
...
:::tip
Wir haben eine ausgewählte Gruppe von Modellen und Anbietern getestet, die gut mit FrenixCode funktionieren.
:::
...
FrenixCode Go gibt dir Zugriff auf diese Modelle für **10 $/Monat**.

---

## Wie es funktioniert

FrenixCode Go funktioniert wie jeder andere Anbieter in FrenixCode.

1. Du meldest dich bei **<a href={console}>FrenixCode Zen</a>** an, abonnierst Go und kopierst deinen API-Schlüssel.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `FrenixCode Go` und fügst deinen API-Schlüssel ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann FrenixCode Go abonnieren.
:::
...
FrenixCode Go beinhaltet die folgenden Limits:
...
FrenixCode Go ist ein Abonnementplan für **10 $/Monat**. Unten stehen die Preise **pro 1 Mio. Token**.
...
| ------------ | ------------ | ------------------------------------------------ | --------------------------- |
| GLM-5        | glm-5        | `https://frenixcode.dev/zen/go/v1/chat/completions` | `@ai-sdk/openai-compatible` |
| Kimi K2.5    | kimi-k2.5    | `https://frenixcode.dev/zen/go/v1/chat/completions` | `@ai-sdk/openai-compatible` |
| MiniMax M2.5 | minimax-m2.5 | `https://frenixcode.dev/zen/go/v1/messages`         | `@ai-sdk/anthropic`         |

Die [Modell-ID](/docs/config/#models) in deiner FrenixCode-Konfiguration verwendet das Format `frenixcode-go/<model-id>`. Zum Beispiel würdest du für Kimi K2.5 `frenixcode-go/kimi-k2.5` in deiner Konfiguration verwenden.
...
Wir haben FrenixCode Go erstellt, um:
...
4. **Keinen Lock-in** zu haben, indem wir dir ermöglichen, jeden anderen Anbieter ebenfalls mit FrenixCode zu nutzen.

...
Sie können externe Tools zu FrenixCode hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. FrenixCode unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [FrenixCode Config](https://frenixcode.dev/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/frenixcode`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
FrenixCode übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird FrenixCode Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von FrenixCode beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `frenixcode mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert FrenixCode die Token sicher in `~/.local/share/frenixcode/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in FrenixCode verfügbar. Sie können sie also wie jedes andere Tool über die FrenixCode-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und FrenixCode mit Ihrem Sentry-Konto zu verbinden.

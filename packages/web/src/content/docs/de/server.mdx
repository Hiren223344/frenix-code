---
title: Server
description: Nutze den frenixcode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `frenixcode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein frenixcode-Client nutzen kann.
...
Setze `FRENIXCODE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `frenixcode`, kann aber mit `FRENIXCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `frenixcode serve` und `frenixcode web`.
...
Wenn du `frenixcode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den frenixcode-Server, um frenixcode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `frenixcode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `frenixcode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die FrenixCode-[IDE](/docs/ide)-Plugins.
...
Der frenixcode-Server stellt folgende APIs bereit.

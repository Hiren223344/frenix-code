...
Mit FrenixCode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet FrenixCode das `frenixcode`-Theme.
...
FrenixCode liefert mehrere eingebaute Themes mit.
...
- FrenixCode optisch ans Terminal anpassen wollen
...
FrenixCode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/frenixcode/themes/*.json` oder `$XDG_CONFIG_HOME/frenixcode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.frenixcode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.frenixcode/themes/*.json`

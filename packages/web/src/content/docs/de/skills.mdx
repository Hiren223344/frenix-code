...
Agent Skills erlauben FrenixCode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
FrenixCode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft FrenixCode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.frenixcode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/frenixcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.frenixcode/skills/git-release/SKILL.md` zum Beispiel so:
...
FrenixCode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `frenixcode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `frenixcode.json`):

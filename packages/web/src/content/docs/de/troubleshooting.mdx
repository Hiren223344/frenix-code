...
Wenn FrenixCode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\frenixcode\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `frenixcode --log-level DEBUG`.

---

## Speicher

frenixcode speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/frenixcode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\frenixcode`
...
FrenixCode Desktop startet im Hintergrund einen lokalen FrenixCode-Server (`frenixcode-cli`-Sidecar).
...
- Nur macOS: `FrenixCode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/frenixcode/frenixcode.jsonc` (oder `~/.config/frenixcode/frenixcode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/frenixcode/frenixcode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\frenixcode\frenixcode.jsonc`
...
FrenixCode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/frenixcode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\frenixcode\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit FrenixCode ihn neu aufbauen kann.

1. Beende FrenixCode Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/frenixcode`
- **Linux**: loesche `~/.cache/frenixcode` (oder fuehre aus: `rm -rf ~/.cache/frenixcode`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\frenixcode`

3. Starte FrenixCode Desktop neu.

---

### Server-Verbindungsprobleme beheben

FrenixCode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `frenixcode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `FRENIXCODE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `FRENIXCODE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt FrenixCode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit FrenixCode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

FrenixCode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer FrenixCode in den OS-Einstellungen aktiviert sind, und
...
1. Beende FrenixCode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von FrenixCode Desktop):

- `frenixcode.settings.dat` (desktop default server URL)
- `frenixcode.global.dat` und `frenixcode.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit FrenixCode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `frenixcode models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\frenixcode`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. frenixcode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\frenixcode`

2. Starte frenixcode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt frenixcode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
frenixcode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).

---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in FrenixCode.
---

import config from "../../../../config.mjs"
export const console = config.console

FrenixCode verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer FrenixCode-Konfiguration.
...
in `~/.local/share/frenixcode/auth.json`.

---

### Konfiguration

Sie können die Anbieter über den Abschnitt `provider` in Ihrem FrenixCode anpassen
config.
...
FrenixCode Zen ist eine Liste von Modellen, die vom FrenixCode-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit FrenixCode funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit FrenixCode Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie FrenixCode aus und gehen Sie zu [frenixcode.dev/auth](https://frenixcode.dev/auth).
...
Es funktioniert wie jeder andere Anbieter in FrenixCode und ist völlig optional.

---

## FrenixCode Go

FrenixCode Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom FrenixCode-Team getestet und verifiziert wurden, dass sie gut mit FrenixCode funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `FrenixCode Go` und gehen Sie zu [frenixcode.dev/auth](https://frenixcode.dev/zen).
...
So verwenden Sie Amazon Bedrock mit FrenixCode:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie FrenixCode ausführen:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY frenixcode

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile frenixcode

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX frenixcode
   ```
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `frenixcode.json`:

   ```json title="frenixcode.json"
   {
     "$schema": "https://frenixcode.dev/config.json",
     "provider": {
       "amazon-bedrock": {
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in FrenixCode wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit FrenixCode ordnungsgemäß funktioniert.
   :::
...
5. Legen Sie Ihren Ressourcennamen als Umgebungsvariable fest:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX frenixcode
   ```
...
5. Legen Sie Ihren Ressourcennamen als Umgebungsvariable fest:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX frenixcode
   ```
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre FrenixCode-Konfiguration hinzufügen.
...
2. Wählen Sie Ihre Authentifizierungsmethode:
   ...
   3. Name: `FrenixCode`, Bereiche: `api`
...
um das Token im FrenixCode-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
FrenixCode verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von FrenixCode
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`frenixcode.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit FrenixCode:
...
So verwenden Sie Google Vertex AI mit FrenixCode:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie FrenixCode ausführen.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id frenixcode
   ```
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über FrenixCode konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
```jsonc title="~/.config/frenixcode/frenixcode.jsonc"
{
  "$schema": "https://frenixcode.dev/config.json",
  "provider": {
    "helicone": {
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [frenixcode-helicone-session](https://github.com/H2Shami/frenixcode-helicone-session)-Plugin, um jede FrenixCode-Konversation automatisch als Sitzung in Helicone zu protokollieren.

```bash
npm install -g frenixcode-helicone-session
```
...
```json title="frenixcode.json"
{
  "plugin": ["frenixcode-helicone-session"]
}
```

Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede FrenixCode-Konversation als separate Sitzung aufgeführt.
...
Sie können FrenixCode für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren

```json title="frenixcode.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://frenixcode.dev/config.json",
  "provider": {
    "llama.cpp": {
...
Sie können FrenixCode über LM Studio für die Verwendung lokaler Modelle konfigurieren.

```json title="frenixcode.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://frenixcode.dev/config.json",
  "provider": {
    "lmstudio": {
...
Sie können FrenixCode über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für FrenixCode konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/frenixcode).
:::

```json title="frenixcode.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://frenixcode.dev/config.json",
  "provider": {
    "ollama": {
...
So verwenden Sie Ollama Cloud mit FrenixCode:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in FrenixCode.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in FrenixCode verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
FrenixCode Zen ist eine Liste getesteter und verifizierter Modelle, die vom FrenixCode-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>FrenixCode Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **FrenixCode Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren FrenixCode API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre FrenixCode-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre FrenixCode-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
3. Geben Sie Ihren JSON-Service-Account-Key ein.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' frenixcode
   ```
...
4. Legen Sie optional die Bereitstellung ID und die Ressourcengruppe fest:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group frenixcode
   ```
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit FrenixCode:
...
Sie können Modelle auch über Ihre FrenixCode-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit FrenixCode verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
3. Geben Sie Ihren API-Schlüssel für den Anbieter ein.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in frenixcode.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Erstellen oder aktualisieren Sie Ihre `frenixcode.json`-Datei in Ihrem Projektverzeichnis:

   ```json title="frenixcode.json" ""myprovider"" {5-15}
   {
     "$schema": "https://frenixcode.dev/config.json",
     "provider": {
       "myprovider": {
...
```json title="frenixcode.json" {9,11,17-20}
{
  "$schema": "https://frenixcode.dev/config.json",
  "provider": {
    "myprovider": {
...
Mithilfe der `limit`-Felder kann FrenixCode erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `frenixcode auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die FrenixCode-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer FrenixCode-Konfiguration übereinstimmt.

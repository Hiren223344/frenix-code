...
description: Erstellen Sie Tools, die der LLM in FrenixCode aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von FrenixCode wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.frenixcode/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/frenixcode/tools/` platzieren.

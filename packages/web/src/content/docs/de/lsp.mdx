---
title: LSP Server
description: FrenixCode lässt sich in Ihren LSP-Server integrieren.
---

FrenixCode lässt sich in Ihr Language Server Protocol (LSP) integrieren, um die Interaktion von LLM mit Ihrer Codebasis zu unterstützen. Es verwendet Diagnosefunktionen, um dem LLM Feedback zu geben.

---

## Integriert

FrenixCode verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
:::note
Sie können LSP-Server-Downloads automatisch deaktivieren, indem Sie die Umgebungsvariable `FRENIXCODE_DISABLE_LSP_DOWNLOAD` auf `true` setzen.
:::
...
Wenn FrenixCode eine Datei öffnet, geschieht Folgendes:
...
Sie können LSP-Server über den Abschnitt `lsp` in Ihrer FrenixCode-Konfiguration anpassen.
...
```json title="frenixcode.json" {5-7}
{
  "$schema": "https://frenixcode.dev/config.json",
  "lsp": {
    "rust": {
...
```json title="frenixcode.json" {5-9}
{
  "$schema": "https://frenixcode.dev/config.json",
  "lsp": {
    "typescript": {

---
title: Plugins
description: Escreva seus próprios plugins para estender o frenixcode.
---

Plugins permitem que você estenda o frenixcode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do frenixcode.
...
- `.frenixcode/plugins/` - Plugins em nível de projeto
- `~/.config/frenixcode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/frenixcode/node_modules/`.
...
1. Configuração global (`~/.config/frenixcode/frenixcode.json`)
2. Configuração do projeto (`frenixcode.json`)
3. Diretório de plugins global (`~/.config/frenixcode/plugins/`)
4. Diretório de plugins do projeto (`.frenixcode/plugins/`)
...
O frenixcode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do frenixcode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o frenixcode.
...
:::note
Se você estiver usando o aplicativo desktop frenixcode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o frenixcode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao frenixcode:
...
O helper `tool` cria uma ferramenta personalizada que o frenixcode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o frenixcode junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://frenixcode.dev/docs/sdk) para detalhes.

---
title: Provedores
description: Usando qualquer provedor LLM no frenixcode.
---

import config from "../../../../config.mjs"
export const console = config.console

frenixcode usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do frenixcode.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/frenixcode/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do frenixcode.
...
FrenixCode Zen é uma lista de modelos fornecidos pela equipe do frenixcode que foram testados e verificados para funcionar bem com o frenixcode. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o FrenixCode Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `FrenixCode Zen` e acesse [frenixcode.dev/zen](https://frenixcode.dev/zen).
...
Funciona como qualquer outro provedor no frenixcode e é completamente opcional.

---

## FrenixCode Go

FrenixCode Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do frenixcode que foram
testados e verificados para funcionar bem com o frenixcode.

1. Execute o comando `/connect` no TUI, selecione `FrenixCode Go` e acesse [frenixcode.dev/zen](https://frenixcode.dev/zen).
...
Para usar o Amazon Bedrock com o frenixcode:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o frenixcode:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `frenixcode.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no frenixcode não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o frenixcode funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do frenixcode.
...
armazenar o token no armazenamento de autenticação do frenixcode.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
frenixcode usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o frenixcode
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `frenixcode.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Personalize através do `frenixcode.json`:
...
Para usar sua assinatura do GitHub Copilot com o frenixcode:
...
Para usar o Google Vertex AI com o frenixcode:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o frenixcode.
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do frenixcode, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [frenixcode-helicone-session](https://github.com/H2Shami/frenixcode-helicone-session) para registrar automaticamente cada conversa do frenixcode como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do frenixcode listada como uma sessão separada.
...
Você pode configurar o frenixcode para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o frenixcode para usar modelos locais através do LM Studio.
...
Você pode configurar o frenixcode para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o frenixcode. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/frenixcode) para detalhes.
:::
...
Para usar o Ollama Cloud com o frenixcode:
...
3. Copie a chave da API para uso no frenixcode.
...
6. **Importante**: Antes de usar modelos em nuvem no frenixcode, você deve puxar as informações do modelo localmente:
...
FrenixCode Zen é uma lista de modelos testados e verificados fornecidos pela equipe do frenixcode. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>FrenixCode Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **FrenixCode Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do frenixcode.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do frenixcode.
...
5. Você também pode personalizá-los através da sua configuração do frenixcode. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o frenixcode:
...
Você também pode personalizar modelos através da sua configuração do frenixcode. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o frenixcode. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no frenixcode.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `frenixcode.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o frenixcode entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `frenixcode auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do frenixcode e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do frenixcode.

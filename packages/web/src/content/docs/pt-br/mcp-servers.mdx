...
Você pode adicionar ferramentas externas ao frenixcode usando o _Model Context Protocol_, ou MCP. O frenixcode suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do frenixcode](https://frenixcode.dev/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/frenixcode`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O frenixcode lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o frenixcode irá:
...
Se o servidor requer autenticação, o frenixcode solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `frenixcode mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o frenixcode armazenará os tokens de forma segura em `~/.local/share/frenixcode/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no frenixcode, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do frenixcode como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o frenixcode à sua conta do Sentry.

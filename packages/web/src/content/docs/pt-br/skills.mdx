...
As habilidades do agente permitem que o frenixcode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O frenixcode pesquisa nesses locais:

- Configuração do projeto: `.frenixcode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/frenixcode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o frenixcode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.frenixcode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/frenixcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.frenixcode/skills/git-release/SKILL.md` assim:
...
O frenixcode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `frenixcode.json`:
...
**Para agentes embutidos** (em `frenixcode.json`):

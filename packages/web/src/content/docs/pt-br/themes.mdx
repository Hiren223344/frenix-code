...
Com o frenixcode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o frenixcode usa nosso próprio tema `frenixcode`.
...
O frenixcode vem com vários temas embutidos.
...
- Querem que o frenixcode corresponda à aparência do seu terminal
...
O frenixcode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/frenixcode/themes/*.json` ou `$XDG_CONFIG_HOME/frenixcode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.frenixcode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.frenixcode/themes/*.json`

...
As ferramentas permitem que o LLM execute ações em sua base de código. O frenixcode vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no frenixcode.
...
:::note
Esta ferramenta está disponível apenas quando `FRENIXCODE_EXPERIMENTAL_LSP_TOOL=true` (ou `FRENIXCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor frenixcode ou quando a variável de ambiente `FRENIXCODE_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o frenixcode:

---
title: Servidor
description: Interaja com o servidor frenixcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `frenixcode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente frenixcode pode usar.
...
```bash
frenixcode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `frenixcode.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
frenixcode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `FRENIXCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `frenixcode`, ou defina `FRENIXCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `frenixcode serve` quanto ao `frenixcode web`.

```bash
FRENIXCODE_SERVER_PASSWORD=your-password frenixcode serve
```
...
Quando você executa `frenixcode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor frenixcode para interagir com o frenixcode programaticamente.
:::

Essa arquitetura permite que o frenixcode suporte múltiplos clientes e permite que você interaja com o frenixcode programaticamente.

Você pode executar `frenixcode serve` para iniciar um servidor autônomo. Se você tiver o TUI do frenixcode em execução, `frenixcode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do frenixcode [IDE](/docs/ide).
...
O servidor frenixcode expõe as seguintes APIs.

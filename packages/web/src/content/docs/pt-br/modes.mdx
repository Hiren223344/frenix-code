...
:::caution
Os modos agora são configurados através da opção `agent` na configuração do frenixcode. A opção `mode` agora está obsoleta. [Saiba mais](/docs/agents).
:::

Os modos no frenixcode permitem que você personalize o comportamento, as ferramentas e os prompts para diferentes casos de uso.

Ele vem com dois modos integrados: **build** e **plan**. Você pode personalizar esses ou configurar os seus próprios através da configuração do frenixcode.
...
O frenixcode vem com dois modos integrados.
...
- `edit` - Não pode modificar arquivos existentes, exceto para arquivos localizados em `.frenixcode/plans/*.md` para detalhar o plano em si
...
Configure os modos no seu arquivo de configuração `frenixcode.json`:
...
- Global: `~/.config/frenixcode/modes/`
- Projeto: `.frenixcode/modes/`

```markdown title="~/.config/frenixcode/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
Se nenhuma temperatura for especificada, o frenixcode usa padrões específicos do modelo (geralmente 0 para a maioria dos modelos, 0.55 para modelos Qwen).
...
Este caminho é relativo a onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do frenixcode quanto para a configuração específica do projeto.
...
```json title="frenixcode.json" {4-14}
{
  "$schema": "https://frenixcode.dev/config.json",
  "mode": {
    "docs": {
...
Crie arquivos de modo em `.frenixcode/modes/` para modos específicos do projeto ou `~/.config/frenixcode/modes/` para modos globais:

```markdown title=".frenixcode/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/frenixcode/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:

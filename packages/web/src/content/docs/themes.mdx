...
With FrenixCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, FrenixCode uses our own `frenixcode` theme.
...
FrenixCode comes with several built-in themes.
...
- Want FrenixCode to match their terminal's appearance
...
FrenixCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/frenixcode/themes/*.json` or `$XDG_CONFIG_HOME/frenixcode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.frenixcode/themes/*.json`
4. **Current working directory** - Defined in `./.frenixcode/themes/*.json`

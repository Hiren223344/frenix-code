---
title: Providers
description: Using any LLM provider in FrenixCode.
---

import config from "../../../config.mjs"
export const console = config.console

FrenixCode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your FrenixCode config.
...
You can customize the providers through the `provider` section in your FrenixCode
config.
...
FrenixCode Zen is a list of models provided by the FrenixCode team that have been
tested and verified to work well with FrenixCode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with FrenixCode Zen.
:::

1. Run the `/connect` command in the TUI, select `FrenixCode Zen`, and head to [frenixcode.dev/auth](https://frenixcode.dev/zen).
...
It works like any other provider in FrenixCode and is completely optional to use.

---

## FrenixCode Go

FrenixCode Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the FrenixCode team that have been
tested and verified to work well with FrenixCode.

1. Run the `/connect` command in the TUI, select `FrenixCode Go`, and head to [frenixcode.dev/auth](https://frenixcode.dev/zen).
...
To use Amazon Bedrock with FrenixCode:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running frenixcode:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `frenixcode.json`:
...
:::info
Using your Claude Pro/Max subscription in FrenixCode is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for frenixcode to work properly.
   :::
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your frenixcode config.
...
FrenixCode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock FrenixCode
to only use your own GitLab-hosted instance, add the following to your
`frenixcode.json` file. It is also recommended to disable session sharing.
...
To use your GitHub Copilot subscription with frenixcode:
...
To use Google Vertex AI with FrenixCode:
...
2. Set the required environment variables:
   ...
   Set them while running frenixcode.
...
In the event you see a feature or model from Helicone that isn't configured automatically through frenixcode, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [frenixcode-helicone-session](https://github.com/H2Shami/frenixcode-helicone-session) plugin to automatically log each FrenixCode conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each FrenixCode conversation listed as a separate session.
...
You can configure frenixcode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure frenixcode to use local models through LM Studio.
...
You can configure frenixcode to use local models through Ollama.

:::tip
Ollama can automatically configure itself for FrenixCode. See the [Ollama integration docs](https://docs.ollama.com/integrations/frenixcode) for details.
:::
...
To use Ollama Cloud with FrenixCode:
...
3. Copy the API key for use in FrenixCode.
...
6. **Important**: Before using cloud models in FrenixCode, you must pull the model information locally:
...
FrenixCode Zen is a list of tested and verified models provided by the FrenixCode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>FrenixCode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **FrenixCode Zen**.

   ```txt
   /connect
   ```

3. Enter your FrenixCode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your frenixcode config.
...
5. You can also customize them through your frenixcode config. Here's an example of specifying a provider
...
You can also customize models through your frenixcode config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with frenixcode. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `frenixcode.json` file in your project directory:
...
The `limit` fields allow FrenixCode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `frenixcode auth list` to see if the credentials
...
2. For custom providers, check the frenixcode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your frenixcode config.

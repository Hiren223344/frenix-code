...
Puede agregar herramientas externas a FrenixCode utilizando el _Model Context Protocol_ o MCP. FrenixCode admite servidores locales y remotos.
...
Puede definir servidores MCP en su [FrenixCode Config](https://frenixcode.dev/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/frenixcode`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
FrenixCode maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, FrenixCode:
...
Si el servidor requiere autenticación, FrenixCode le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `frenixcode mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, FrenixCode almacenará los tokens de forma segura en `~/.local/share/frenixcode/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en FrenixCode, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración FrenixCode como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar FrenixCode a su cuenta Sentry.

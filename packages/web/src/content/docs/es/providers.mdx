---
title: Proveedores
description: Usando cualquier proveedor LLM en FrenixCode.
---

import config from "../../../../config.mjs"
export const console = config.console

FrenixCode usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración FrenixCode.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu FrenixCode
configuración.
...
FrenixCode Zen es una lista de modelos proporcionados por el equipo FrenixCode que han sido
probado y verificado para funcionar bien con FrenixCode. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con FrenixCode Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione frenixcode y diríjase a [frenixcode.dev/auth](https://frenixcode.dev/auth).
...
Funciona como cualquier otro proveedor en FrenixCode y su uso es completamente opcional.

---

## FrenixCode Go

FrenixCode Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de FrenixCode que han sido
probado y verificado para funcionar bien con FrenixCode.

1. Ejecute el comando `/connect` en TUI, seleccione `FrenixCode Go` y diríjase a [frenixcode.dev/auth](https://frenixcode.dev/zen).
...
Para usar Amazon Bedrock con FrenixCode:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta frenixcode:
   ...
   Para una configuración persistente o específica del proyecto, utilice `frenixcode.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en FrenixCode no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que frenixcode funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración frenixcode.
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `FrenixCode`, Alcances: `api`
...
para almacenar el token en el almacenamiento de autenticación frenixcode.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
FrenixCode utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear FrenixCode
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `frenixcode.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `frenixcode.json`:
...
Para utilizar su suscripción GitHub Copilot con frenixcode:
...
Para utilizar Google Vertex AI con FrenixCode:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta frenixcode.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de frenixcode, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [frenixcode-helicone-session](https://github.com/H2Shami/frenixcode-helicone-session) para registrar automáticamente cada conversación FrenixCode como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación FrenixCode enumerada como una sesión separada.
...
Puede configurar frenixcode para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar frenixcode para usar modelos locales a través de LM Studio.
...
Puedes configurar frenixcode para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para FrenixCode. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/frenixcode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con FrenixCode:
...
3. Copie la clave API para usarla en FrenixCode.
...
6. **Importante**: Antes de usar modelos en la nube en FrenixCode, debe obtener la información del modelo localmente:
...
FrenixCode Zen es una lista de modelos probados y verificados proporcionada por el equipo FrenixCode. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>FrenixCode Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **FrenixCode Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave FrenixCode API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración frenixcode.
...
5. También puedes personalizarlos a través de tu configuración frenixcode. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con FrenixCode:
...
También puede personalizar modelos a través de su configuración frenixcode. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con frenixcode. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `frenixcode.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a FrenixCode comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `frenixcode auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración frenixcode y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración frenixcode.

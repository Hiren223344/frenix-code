---
title: Complementos
description: Escriba sus propios complementos para extender FrenixCode.
---

Los complementos le permiten extender FrenixCode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de FrenixCode.
...
- `.frenixcode/plugins/` - Complementos a nivel de proyecto
- `~/.config/frenixcode/plugins/` - Complementos globales
...
```json title="frenixcode.json"
{
  ...
  "plugin": ["frenixcode-helicone-session", "frenixcode-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/frenixcode/node_modules/`.
...
1. Configuración global (`~/.config/frenixcode/frenixcode.json`)
2. Configuración del proyecto (`frenixcode.json`)
3. Directorio global de complementos (`~/.config/frenixcode/plugins/`)
4. Directorio de complementos del proyecto (`.frenixcode/plugins/`)
...
```json title=".frenixcode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
FrenixCode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".frenixcode/plugins/my-plugin.ts"
...
```js title=".frenixcode/plugins/example.js"
...
- `client`: Un cliente SDK frenixcode para interactuar con la IA.
...
import type { Plugin } from "@frenixcode/plugin"
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar frenixcode.
...
```js title=".frenixcode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "frenixcode"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio FrenixCode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que frenixcode lea archivos `.env`:

```javascript title=".frenixcode/plugins/env-protection.js"
...
```javascript title=".frenixcode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a frenixcode:

```ts title=".frenixcode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@frenixcode/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que frenixcode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para frenixcode junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://frenixcode.dev/docs/sdk) para obtener más detalles.
...
```ts title=".frenixcode/plugins/compaction.ts"
import type { Plugin } from "@frenixcode/plugin"
...
```ts title=".frenixcode/plugins/custom-compaction.ts"
import type { Plugin } from "@frenixcode/plugin"

...
FrenixCode Go es una suscripción de bajo coste de **10 $/mes** que te ofrece acceso fiable a modelos populares de código abierto.

:::note
FrenixCode Go está actualmente en beta.
:::

Go funciona como cualquier otro proveedor en FrenixCode. Te suscribes a FrenixCode Go y obtienes tu clave API. Es **completamente opcional** y no necesitas usarlo para utilizar FrenixCode.
...
:::tip
Hemos probado un grupo selecto de modelos y proveedores que funcionan bien con FrenixCode.
:::
...
FrenixCode Go te da acceso a estos modelos por **10 $/mes**.

---

## Cómo funciona

FrenixCode Go funciona como cualquier otro proveedor en FrenixCode.

1. Inicias sesión en **<a href={console}>FrenixCode Zen</a>**, te suscribes a Go y copias tu clave API.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `FrenixCode Go` y pegas tu clave API.
3. Ejecuta `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a FrenixCode Go.
:::
...
FrenixCode Go incluye los siguientes límites:
...
FrenixCode Go es un plan de suscripción de **10 $/mes**. A continuación se muestran los precios **por 1M de tokens**.
...
El [model id](/docs/config/#models) en tu configuración de FrenixCode usa el formato `frenixcode-go/<model-id>`. Por ejemplo, para Kimi K2.5, usarías `frenixcode-go/kimi-k2.5` en tu configuración.
...
4. **Sin ataduras**, permitiéndote usar cualquier otro proveedor con FrenixCode también.

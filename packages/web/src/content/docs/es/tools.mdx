...
Las herramientas permiten que LLM realice acciones en su código base. FrenixCode viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en FrenixCode.
...
:::note
Esta herramienta solo está disponible cuando `FRENIXCODE_EXPERIMENTAL_LSP_TOOL=true` (o `FRENIXCODE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor FrenixCode o cuando la variable de entorno `FRENIXCODE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar FrenixCode:

```bash
FRENIXCODE_ENABLE_EXA=1 frenixcode
```

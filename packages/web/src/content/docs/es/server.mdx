---
title: Servidor
description: Interactuar con el servidor frenixcode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `frenixcode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente frenixcode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `frenixcode.local` |
...
Configure `FRENIXCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `frenixcode`, o configure `FRENIXCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `frenixcode serve` como a `frenixcode web`.
...
Cuando ejecuta `frenixcode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor frenixcode para interactuar con frenixcode mediante programación.
:::

Esta arquitectura permite que frenixcode admita múltiples clientes y le permite interactuar con frenixcode mediante programación.

Puede ejecutar `frenixcode serve` para iniciar un servidor independiente. Si tienes el
frenixcode TUI ejecutándose, `frenixcode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos FrenixCode [IDE](/docs/ide).
...
El servidor frenixcode expone las siguientes API.

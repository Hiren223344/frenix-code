...
Las habilidades del agente permiten a FrenixCode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
FrenixCode busca estas ubicaciones:

- Configuración del proyecto: `.frenixcode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/frenixcode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, FrenixCode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.frenixcode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/frenixcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.frenixcode/skills/git-release/SKILL.md` así:
...
FrenixCode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `frenixcode.json`:
...
**Para agentes integrados** (en `frenixcode.json`):

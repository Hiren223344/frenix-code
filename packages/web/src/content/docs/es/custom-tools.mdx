...
description: Cree herramientas que LLM pueda llamar en frenixcode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de frenixcode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.frenixcode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/frenixcode/tools/`.
...
```ts title=".frenixcode/tools/database.ts" {1}
...
```ts title=".frenixcode/tools/math.ts"
import { tool } from "@frenixcode/plugin"
...
```ts title=".frenixcode/tools/bash.ts"
import { tool } from "@frenixcode/plugin"
...
```ts title=".frenixcode/tools/project.ts" {8}
import { tool } from "@frenixcode/plugin"
...
```python title=".frenixcode/tools/add.py"
import sys
...
```ts title=".frenixcode/tools/python-add.ts" {10}
import { tool } from "@frenixcode/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".frenixcode/tools/add.py")

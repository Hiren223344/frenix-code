...
Con FrenixCode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, FrenixCode usa nuestro propio tema `frenixcode`.
...
FrenixCode viene con varios temas integrados.
...
- Quiere que FrenixCode coincida con la apariencia de su terminal
...
FrenixCode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/frenixcode/themes/*.json` o `$XDG_CONFIG_HOME/frenixcode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.frenixcode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.frenixcode/themes/*.json`

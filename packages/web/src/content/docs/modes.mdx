...
:::caution
Modes are now configured through the `agent` option in the frenixcode config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in frenixcode allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the frenixcode config.
...
frenixcode comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.frenixcode/plans/*.md` to detail the plan itself
...
Configure modes in your `frenixcode.json` config file:
...
both the global frenixcode config and the project specific config.
...
Create mode files in `.frenixcode/modes/` for project-specific modes or `~/.config/frenixcode/modes/` for global modes:

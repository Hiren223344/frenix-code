...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di frenixcode. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in frenixcode ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di frenixcode.
...
frenixcode include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.frenixcode/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `frenixcode.json`:
...
- Globale: `~/.config/frenixcode/modes/`
- Progetto: `.frenixcode/modes/`
...
Se non viene specificata alcuna temperatura, frenixcode usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di frenixcode sia per quella specifica del progetto.
...
Crea file di modalita in `.frenixcode/modes/` per modalita specifiche del progetto oppure in `~/.config/frenixcode/modes/` per modalita globali:

...
FrenixCode Go è un abbonamento a basso costo di **$10/mese** che ti offre un accesso affidabile ai modelli di coding open source più popolari.

:::note
FrenixCode Go è attualmente in beta.
:::

Go funziona come qualsiasi altro provider in FrenixCode. Ti abboni a FrenixCode Go e ottieni la tua chiave API. È **completamente opzionale** e non è necessario utilizzarlo per usare FrenixCode.
...
FrenixCode Go ti dà accesso a questi modelli per **$10/mese**.

---

## Come funziona

FrenixCode Go funziona come qualsiasi altro provider in FrenixCode.

1. Accedi a **<a href={console}>FrenixCode Zen</a>**, abbonati a Go e copia la tua chiave API.
2. Esegui il comando `/connect` nella TUI, seleziona `FrenixCode Go` e incolla la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a FrenixCode Go.
:::
...
FrenixCode Go include i seguenti limiti:
...
FrenixCode Go è un piano di abbonamento da **$10/mese**. Di seguito sono riportati i prezzi **per 1M di token**.
...
Il [model id](/docs/config/#models) nella tua configurazione FrenixCode utilizza il formato `frenixcode-go/<model-id>`. Ad esempio, per Kimi K2.5, useresti `frenixcode-go/kimi-k2.5` nella tua configurazione.
...
Abbiamo creato FrenixCode Go per:
...
4. Non avere **alcun lock-in** permettendoti di utilizzare qualsiasi altro provider con FrenixCode.

...
description: Usa la configurazione JSON di FrenixCode.
---

Puoi configurare FrenixCode usando un file di configurazione JSON.

---

## Formato

FrenixCode supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
1. **Config remota** (da `.well-known/frenixcode`) - default dell'organizzazione
2. **Config globale** (`~/.config/frenixcode/frenixcode.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `FRENIXCODE_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`frenixcode.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.frenixcode`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `FRENIXCODE_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.frenixcode` e `~/.config/frenixcode` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/frenixcode`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di FrenixCode in `~/.config/frenixcode/frenixcode.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/frenixcode/tui.json`.
...
Aggiungi `frenixcode.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando FrenixCode si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `FRENIXCODE_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `FRENIXCODE_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.frenixcode` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.frenixcode`, quindi **puo sovrascrivere** le loro impostazioni.

---

## Schema

Il file di configurazione ha uno schema definito in [**`frenixcode.dev/config.json`**](https://frenixcode.dev/config.json).

La configurazione TUI usa [**`frenixcode.dev/tui.json`**](https://frenixcode.dev/tui.json).
...
Usa `FRENIXCODE_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `frenixcode.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `frenixcode serve` e `frenixcode web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server FrenixCode.
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `frenixcode.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi configurare provider e modelli da usare in FrenixCode tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, FrenixCode prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi anche definire agenti usando file markdown in `~/.config/frenixcode/agents/` o `.frenixcode/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, FrenixCode fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`frenixcode run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/frenixcode/commands/` o `.frenixcode/commands/`. [Scopri di piu](/docs/commands).
...
FrenixCode scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, frenixcode **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono FrenixCode con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.frenixcode/plugins/` o `~/.config/frenixcode/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare FrenixCode a usare solo alcuni provider invece di disabilitarli uno per uno.

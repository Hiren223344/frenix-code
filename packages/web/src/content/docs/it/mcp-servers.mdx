...
Puoi aggiungere strumenti esterni a FrenixCode usando il _Model Context Protocol_, o MCP. FrenixCode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [FrenixCode Config](https://frenixcode.dev/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/frenixcode`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
FrenixCode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, FrenixCode:
...
Se il server richiede autenticazione, FrenixCode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `frenixcode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, FrenixCode memorizzera' i token in modo sicuro in `~/.local/share/frenixcode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in FrenixCode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione FrenixCode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare FrenixCode al tuo account Sentry.

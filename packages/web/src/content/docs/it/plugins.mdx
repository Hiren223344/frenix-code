---
title: Plugin
description: Scrivi plugin per estendere FrenixCode.
---

I plugin ti permettono di estendere FrenixCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di FrenixCode.
...
- `.frenixcode/plugins/` - plugin a livello progetto
- `~/.config/frenixcode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/frenixcode/node_modules/`.
...
1. Config globale (`~/.config/frenixcode/frenixcode.json`)
2. Config di progetto (`frenixcode.json`)
3. Directory plugin globale (`~/.config/frenixcode/plugins/`)
4. Directory plugin di progetto (`.frenixcode/plugins/`)
...
FrenixCode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di frenixcode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere frenixcode.
...
:::note
Se usi l'app desktop di FrenixCode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a frenixcode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a frenixcode:
...
L'helper `tool` crea uno strumento personalizzato che frenixcode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in frenixcode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://frenixcode.dev/docs/sdk) per i dettagli.

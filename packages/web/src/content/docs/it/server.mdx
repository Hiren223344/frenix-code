---
title: Server
description: Interagisci con il server frenixcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `frenixcode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client frenixcode.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `frenixcode.local` |
...
Imposta `FRENIXCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `frenixcode`, oppure imposta `FRENIXCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `frenixcode serve` sia per `frenixcode web`.
...
Quando esegui `frenixcode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server frenixcode per interagire con frenixcode in modo programmatico.
:::

Questa architettura permette a frenixcode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `frenixcode serve` per avviare un server standalone. Se la TUI di frenixcode e gia in esecuzione, `frenixcode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di FrenixCode.
...
Il server frenixcode espone le seguenti API.

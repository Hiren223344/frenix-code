...
Le skill degli agenti permettono a FrenixCode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
FrenixCode cerca in queste posizioni:

- Config di progetto: `.frenixcode/skills/<name>/SKILL.md`
- Config globale: `~/.config/frenixcode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, FrenixCode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.frenixcode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/frenixcode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.frenixcode/skills/git-release/SKILL.md` cosi':
...
FrenixCode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `frenixcode.json`:
...
**Per agenti integrati** (in `frenixcode.json`):

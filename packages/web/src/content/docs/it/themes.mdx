...
Con FrenixCode puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, FrenixCode usa il tema `frenixcode`.
...
FrenixCode include diversi temi integrati.
...
- Vuole che FrenixCode corrisponda all'aspetto del terminale
...
FrenixCode supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/frenixcode/themes/*.json` o `$XDG_CONFIG_HOME/frenixcode/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.frenixcode/themes/*.json`
4. **Directory di lavoro corrente** - in `./.frenixcode/themes/*.json`

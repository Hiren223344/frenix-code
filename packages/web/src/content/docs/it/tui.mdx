---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di FrenixCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

FrenixCode fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire FrenixCode avvia la TUI per la directory corrente.
...
Quando usi la TUI di FrenixCode, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a FrenixCode. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da FrenixCode. _Alias_: `/quit`, `/q`
...
Questo è separato da `frenixcode.json`, che configura il comportamento di server/runtime.
...
Usa `FRENIXCODE_TUI_CONFIG` per caricare un path di configurazione TUI personalizzato.

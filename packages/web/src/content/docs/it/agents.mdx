...
In FrenixCode esistono due tipi di agenti: agenti primari e subagenti.
...
FrenixCode include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
FrenixCode include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

FrenixCode include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `frenixcode.json`:
...
- Globale: `~/.config/frenixcode/agents/`
- Per progetto: `.frenixcode/agents/`
...
Se non specifichi una temperature, FrenixCode usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di FrenixCode sia per la config specifica del progetto.
...
L'ID modello nella configurazione FrenixCode usa il formato `provider/model-id`. Per esempio, se stai usando [FrenixCode Zen](/docs/zen), useresti `frenixcode/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `frenixcode models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/Hiren223344/frenix-code).
:::

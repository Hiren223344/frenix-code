---
title: Plugins
description: Écrivez vos propres plugins pour étendre FrenixCode.
---

Les plugins vous permettent d'étendre FrenixCode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de FrenixCode.
...
- `.frenixcode/plugins/` - Plugins au niveau du projet
- `~/.config/frenixcode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/frenixcode/node_modules/`.
...
1. Configuration globale (`~/.config/frenixcode/frenixcode.json`)
2. Configuration du projet (`frenixcode.json`)
3. Répertoire global des plugins (`~/.config/frenixcode/plugins/`)
4. Répertoire des plugins du projet (`.frenixcode/plugins/`)
...
FrenixCode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK frenixcode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre frenixcode.
...
:::note
Si vous utilisez l'application de bureau FrenixCode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez frenixcode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à frenixcode :
...
L'assistant `tool` crée un outil personnalisé que frenixcode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour frenixcode aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://frenixcode.dev/docs/sdk) pour plus de détails.

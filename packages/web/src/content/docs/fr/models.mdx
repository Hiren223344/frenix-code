...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez FrenixCode.
...
Voici plusieurs modèles qui fonctionnent bien avec FrenixCode, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration FrenixCode.

```json title="frenixcode.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [FrenixCode Zen](/docs/zen), vous utiliserez `frenixcode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="frenixcode.jsonc" {7-12,19-24}
{
  "$schema": "https://frenixcode.dev/config.json",
  "provider": {
    "openai": {
...
```jsonc title="frenixcode.jsonc" {6-21}
{
  "$schema": "https://frenixcode.dev/config.json",
  "provider": {
    "frenixcode": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. FrenixCode est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

FrenixCode est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="frenixcode.jsonc" {7-18}
{
  "$schema": "https://frenixcode.dev/config.json",
  "provider": {
    "openai": {
...
Lorsque FrenixCode démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration FrenixCode.

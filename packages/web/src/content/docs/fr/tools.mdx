...
Les outils permettent au LLM d'effectuer des actions dans votre base de code. FrenixCode est livré avec un ensemble d'outils intégrés, mais vous pouvez l'étendre avec [outils personnalisés](/docs/custom-tools) ou [serveurs MCP](/docs/mcp-servers).
...
Voici tous les outils intégrés disponibles dans FrenixCode.
...
:::note
Cet outil n'est disponible que lorsque `FRENIXCODE_EXPERIMENTAL_LSP_TOOL=true` (ou `FRENIXCODE_EXPERIMENTAL=true`).
:::
...
:::note
Cet outil n'est disponible que lors de l'utilisation du fournisseur FrenixCode ou lorsque la variable d'environnement `FRENIXCODE_ENABLE_EXA` est définie sur une valeur véridique (par exemple, `true` ou `1`).

À activer lors du lancement de FrenixCode :

...
Il existe deux types d'agents dans FrenixCode ; agents primaires et sous-agents.
...
FrenixCode est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
FrenixCode est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

FrenixCode est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `frenixcode.json` :
...
- Global : `~/.config/frenixcode/agents/`
- Par projet : `.frenixcode/agents/`

```markdown title="~/.config/frenixcode/agents/review.md"
---
...
Si aucune température n'est spécifiée, FrenixCode utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale FrenixCode et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration FrenixCode utilise le format `provider/model-id`. Par exemple, si vous utilisez [FrenixCode Zen](/docs/zen), vous utiliserez `frenixcode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="frenixcode.json" {3-6,9-12}
{
  "$schema": "https://frenixcode.dev/config.json",
  "tools": {
    "write": true,
...
```json title="frenixcode.json" {3-5,8-10}
{
  "$schema": "https://frenixcode.dev/config.json",
  "permission": {
    "edit": "deny"
...
```json title="frenixcode.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Exécutez `frenixcode models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/Hiren223344/frenix-code).
:::
...
```markdown title="~/.config/frenixcode/agents/docs-writer.md"
---
...
```markdown title="~/.config/frenixcode/agents/security-auditor.md"
---

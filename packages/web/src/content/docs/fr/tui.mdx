---
title: TUI
description: Utilisation de l’interface utilisateur du terminal FrenixCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

FrenixCode fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de FrenixCode démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le FrenixCode TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à FrenixCode. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Quittez FrenixCode. *Alias* : `/quit`, `/q`
...
Ceci est séparé de `frenixcode.json`, qui configure le comportement du serveur/d'exécution.
...
Utilisez `FRENIXCODE_TUI_CONFIG` pour charger un chemin de configuration TUI personnalisé.

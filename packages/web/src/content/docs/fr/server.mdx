---
title: Serveur
description: Interagissez avec le serveur frenixcode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `frenixcode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client frenixcode peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `frenixcode.local` |
...
Définissez `FRENIXCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `frenixcode`, ou définissez `FRENIXCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `frenixcode serve` et à `frenixcode web`.
...
Lorsque vous exécutez `frenixcode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur frenixcode pour interagir avec frenixcode par programmation.
:::

Cette architecture permet à frenixcode de prendre en charge plusieurs clients et vous permet d'interagir avec frenixcode par programme.

Vous pouvez exécuter `frenixcode serve` pour démarrer un serveur autonome. Si vous avez le
frenixcode TUI en cours d'exécution, `frenixcode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins FrenixCode [IDE](/docs/ide).
...
Le serveur frenixcode expose les API suivantes.

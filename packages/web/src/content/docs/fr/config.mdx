---
title: Configuration
description: Utilisation de la configuration JSON FrenixCode.
---

Vous pouvez configurer FrenixCode à l'aide d'un fichier de configuration JSON.

---

## Format

FrenixCode prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).

```jsonc title="frenixcode.jsonc"
{
...
1. **Configuration distante** (à partir de `.well-known/frenixcode`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/frenixcode/frenixcode.json`) - préférences utilisateur
3. **Configuration personnalisée** (`FRENIXCODE_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`frenixcode.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.frenixcode`** - agents, commandes, plugins
6. **Configuration en ligne** (`FRENIXCODE_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.frenixcode` et `~/.config/frenixcode` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/frenixcode`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
```json title="Remote config from .well-known/frenixcode"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Placez votre configuration globale FrenixCode dans `~/.config/frenixcode/frenixcode.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/frenixcode/tui.json`.
...
Ajoutez `frenixcode.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Lorsque FrenixCode démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `FRENIXCODE_CONFIG`.

```bash
export FRENIXCODE_CONFIG=/path/to/my/custom-config.json
frenixcode run "Hello world"
```
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `FRENIXCODE_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.frenixcode`, et devrait suivre la même structure.

```bash
export FRENIXCODE_CONFIG_DIR=/path/to/my/config-directory
frenixcode run "Hello world"
```

Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.frenixcode`, il **peut donc remplacer** leurs paramètres.

---

## Schéma

Le fichier de configuration a un schéma défini dans [**`frenixcode.dev/config.json`**](https://frenixcode.dev/config.json).

La configuration TUI utilise [**`frenixcode.dev/tui.json`**](https://frenixcode.dev/tui.json).
...
```json title="tui.json"
{
  "$schema": "https://frenixcode.dev/tui.json",
  "scroll_speed": 3,
  "scroll_acceleration": {
    "enabled": true
  },
...
Utilisez `FRENIXCODE_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `frenixcode.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `frenixcode serve` et `frenixcode web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur FrenixCode.
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `frenixcode.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration FrenixCode via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, FrenixCode essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/frenixcode/agents/` ou `.frenixcode/agents/`. [En savoir plus ici](/docs/agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, FrenixCode reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`frenixcode run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/frenixcode/commands/` ou `.frenixcode/commands/`. [En savoir plus ici](/docs/commands).
...
FrenixCode téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
Par défaut, frenixcode **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
[Plugins](/docs/plugins) étendent FrenixCode avec des outils, des hooks et des intégrations personnalisés.

Placez les fichiers du plugin dans `.frenixcode/plugins/` ou `~/.config/frenixcode/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.

```json title="frenixcode.json"
{
  "$schema": "https://frenixcode.dev/config.json",
  "plugin": ["frenixcode-helicone-session", "@my-org/custom-plugin"]
}
```
...
Ceci est utile lorsque vous souhaitez restreindre FrenixCode à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.

...
Avec FrenixCode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, FrenixCode utilise notre propre thème `frenixcode`.
...
FrenixCode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que FrenixCode corresponde à l'apparence de leur terminal
...
FrenixCode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/frenixcode/themes/*.json` ou `$XDG_CONFIG_HOME/frenixcode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.frenixcode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.frenixcode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/frenixcode/themes
vim ~/.config/frenixcode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .frenixcode/themes
vim .frenixcode/themes/my-theme.json
```
...
{
  "$schema": "https://frenixcode.dev/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",

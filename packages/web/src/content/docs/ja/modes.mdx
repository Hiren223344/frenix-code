...
:::caution
モードは、FrenixCode 設定の `agent` オプションを通じて設定されるようになりました。
...
これらを使用するか、FrenixCode 設定を通じて独自の設定を行います。
...
FrenixCode には 2 つの組み込みモードが付属しています。
...
- `edit` - 計画自体の詳細を示す `.frenixcode/plans/*.md` にあるファイルを除き、既存のファイルを変更できません
- `patch` - パッチを適用できません
...
`frenixcode.json` 設定ファイルでモードを構成します。
...
- グローバル: `~/.config/frenixcode/modes/`
- プロジェクト: `.frenixcode/modes/`
...
温度が指定されていない場合、FrenixCode はモデル固有のデフォルトを使用します (通常、ほとんどのモデルでは 0、Qwen モデルでは 0.55)。
...
モードファイルをプロジェクト固有モードの場合は `.frenixcode/modes/` に、グローバルモードの場合は `~/.config/frenixcode/modes/` に作成します。

---
title: TUI
description: FrenixCode ターミナルユーザーインターフェイスの使用。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

FrenixCode は、LLM を使用してプロジェクトを作業するための対話型ターミナルインターフェイスまたは TUI を提供します。

FrenixCode を実行すると、現在のディレクトリの TUI が開始されます。
...
FrenixCode TUI を使用する場合、「`/`」に続いてコマンド名を入力すると、アクションをすばやく実行できます。例えば：
...
FrenixCode にプロバイダーを追加します。利用可能なプロバイダーから選択し、その API キーを追加できます。
...
FrenixCode を終了します。 _エイリアス_: `/quit`、`/q`
...
これは、サーバー/ランタイムの動作を構成する `frenixcode.json` とは別です。
...
カスタム TUI 設定パスをロードするには、`FRENIXCODE_TUI_CONFIG` を使用します。

---
title: TUI
description: Using the FrenixCode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

FrenixCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running FrenixCode starts the TUI for the current directory.
...
When using the FrenixCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to FrenixCode. Allows you to select from available providers and add their API keys.
...
Exit FrenixCode. _Aliases_: `/quit`, `/q`
...
This is separate from `frenixcode.json`, which configures server/runtime behavior.
...
Use `FRENIXCODE_TUI_CONFIG` to load a custom TUI config path.

...
Det er to typer agenter i FrenixCode; primære agenter og underagenter.
...
FrenixCode kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
FrenixCode kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

FrenixCode kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `frenixcode.json`:
...
- Globalt: `~/.config/frenixcode/agents/`
- Per prosjekt: `.frenixcode/agents/`
...
Hvis ingen temperatur er spesifisert, bruker FrenixCode modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale FrenixCode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i FrenixCode-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [FrenixCode Zen](/docs/zen), vil du bruke `frenixcode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `frenixcode models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/Hiren223344/frenix-code).
:::

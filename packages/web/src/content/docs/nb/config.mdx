---
title: Konfigurasjon
description: Ved å bruke frenixcode JSON-konfigurasjonen.
---

Du kan konfigurere FrenixCode ved å bruke en JSON konfigurasjonsfil.

---

## Format

FrenixCode støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "frenixcode"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/frenixcode`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/frenixcode/frenixcode.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`FRENIXCODE_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`frenixcode.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.frenixcode` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`FRENIXCODE_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.frenixcode` og `~/.config/frenixcode` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/frenixcode`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale FrenixCode-konfigurasjon i `~/.config/frenixcode/frenixcode.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `frenixcode.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når FrenixCode starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `FRENIXCODE_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `FRENIXCODE_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.frenixcode` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.frenixcode`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`frenixcode.dev/config.json`**](https://frenixcode.dev/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `frenixcode serve` og `frenixcode web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage FrenixCode-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `frenixcode.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i FrenixCode-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver FrenixCode å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/frenixcode/agents/` eller `.frenixcode/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil FrenixCode falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`frenixcode run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/frenixcode/commands/` eller `.frenixcode/commands/`. [Les mer her](/docs/commands).
...
FrenixCode vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater FrenixCode alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider FrenixCode med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.frenixcode/plugins/` eller `~/.config/frenixcode/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense FrenixCode til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.

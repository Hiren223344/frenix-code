---
title: LSP-servere
description: frenixcode integreres med LSP-serverne dine.
---

FrenixCode integreres med din Language Server Protocol (LSP) for å hjelpe LLM med å samhandle med kodebasen din. Den bruker diagnostikk for å gi tilbakemelding til LLM.

---

## Innebygde servere

FrenixCode kommer med flere innebygde LSP-servere for populære språk:
...
:::note
Du kan deaktivere automatiske LSP-servernedlastinger ved å sette miljøvariabelen `FRENIXCODE_DISABLE_LSP_DOWNLOAD` til `true`.
:::
...
Når frenixcode åpner en fil, gjør den:
...
Du kan tilpasse LSP-servere gjennom `lsp`-delen i FrenixCode-konfigurasjonen.

---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i FrenixCode.
---

import config from "../../../../config.mjs"
export const console = config.console

FrenixCode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i FrenixCode-konfigurasjonen.
...
i `~/.local/share/frenixcode/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i FrenixCode
konfig.
...
FrenixCode Zen er en liste over modeller levert av FrenixCode-teamet som har vært
testet og verifisert for å fungere godt med FrenixCode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med FrenixCode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `FrenixCode Zen` og gå til [frenixcode.dev/auth](https://frenixcode.dev/zen).
...
Det fungerer som alle andre leverandører i FrenixCode og er helt valgfritt å bruke.

---

## FrenixCode Go

FrenixCode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av FrenixCode-teamet som har vært
testet og verifisert for å fungere godt med FrenixCode.

1. Kjør kommandoen `/connect` i TUI, velg `FrenixCode Go`, og gå til [frenixcode.dev/auth](https://frenixcode.dev/zen).
...
Slik bruker du Amazon Bedrock med FrenixCode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører FrenixCode:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `frenixcode.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i frenixcode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at FrenixCode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom FrenixCode-konfigurasjonen.
...
for å lagre token i FrenixCode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
FrenixCode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse FrenixCode
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`frenixcode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Tilpass gjennom `frenixcode.json`:
...
Slik bruker du GitHub Copilot-abonnementet med FrenixCode:
...
Slik bruker du Google Vertex AI med FrenixCode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører FrenixCode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom FrenixCode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [frenixcode-helicone-session](https://github.com/H2Shami/frenixcode-helicone-session) plugin for automatisk å logge hver FrenixCode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver FrenixCode-samtale oppført som en separat økt.
...
Du kan konfigurere FrenixCode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere FrenixCode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere FrenixCode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for FrenixCode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/frenixcode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med FrenixCode:
...
3. Kopier API-nøkkelen for bruk i FrenixCode.
...
6. **Viktig**: Før du bruker skymodeller i FrenixCode, må du hente modellinformasjonen lokalt:
...
FrenixCode Zen er en liste over testede og verifiserte modeller levert av FrenixCode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>FrenixCode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **FrenixCode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn FrenixCode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom FrenixCode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom FrenixCode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med FrenixCode:
...
Du kan også tilpasse modeller gjennom FrenixCode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med FrenixCode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `frenixcode.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar FrenixCode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `frenixcode auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk FrenixCode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i FrenixCode-konfigurasjonen.

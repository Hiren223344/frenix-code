...
description: Skriv dine egne programtillegg for å utvide FrenixCode.
---

Plugins lar deg utvide FrenixCode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til FrenixCode.
...
- `.frenixcode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/frenixcode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/frenixcode/frenixcode.json`)
2. Prosjektkonfigurasjon (`frenixcode.json`)
3. Global plugin-katalog (`~/.config/frenixcode/plugins/`)
4. Prosjektpluginkatalog (`.frenixcode/plugins/`)
...
FrenixCode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En FrenixCode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide FrenixCode.
...
:::note
Hvis du bruker FrenixCode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre FrenixCode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til FrenixCode:
...
`tool`-hjelperen lager et tilpasset verktøy som FrenixCode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for FrenixCode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://frenixcode.dev/docs/sdk) for detaljer.

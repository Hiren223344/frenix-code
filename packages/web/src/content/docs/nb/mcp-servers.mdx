...
Du kan legge til eksterne verktøy i FrenixCode ved å bruke _Model Context Protocol_ eller MCP. FrenixCode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [FrenixCode Config](https://frenixcode.dev/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="frenixcode.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/frenixcode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="frenixcode.jsonc" {15}
{
  "$schema": "https://frenixcode.dev/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
FrenixCode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil FrenixCode:
...
Hvis serveren krever autentisering, vil FrenixCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `frenixcode mcp auth <server-name>`.
...
```json title="frenixcode.json" {7-11}
{
  "$schema": "https://frenixcode.dev/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
frenixcode mcp auth my-oauth-server
```
...
```bash
frenixcode mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil FrenixCode lagre tokens sikkert i `~/.local/share/frenixcode/mcp-auth.json`.
...
frenixcode mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i FrenixCode, sammen med innebygde verktøy. Så du kan administrere dem gjennom FrenixCode-konfigurasjonen som alle andre verktøy.
...
```json title="frenixcode.json" {11, 14-18}
{
  "$schema": "https://frenixcode.dev/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="frenixcode.json" {4-8}
{
  "$schema": "https://frenixcode.dev/config.json",
  "mcp": {
    "sentry": {
...
```bash
frenixcode mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble FrenixCode til Sentry-kontoen din.
...
```json title="frenixcode.json" {4-7}
{
  "$schema": "https://frenixcode.dev/config.json",
  "mcp": {
    "context7": {
...
```json title="frenixcode.json" {7-9}
{
  "$schema": "https://frenixcode.dev/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver FrenixCode-samtale oppført som en separat økt.

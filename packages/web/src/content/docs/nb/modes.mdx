...
:::caution
Moduser konfigureres nå gjennom alternativet `agent` i FrenixCode-konfigurasjonen.
Alternativet `mode` er nå utdatert. [Finn ut mer](/docs/agents).
:::

Moduser i FrenixCode lar deg tilpasse oppførselen, verktøyene og prompter for ulike brukstilfeller.

Den kommer med to innebygde moduser: **bygg** og **plan**. Du kan tilpasse
disse eller konfigurer din egen gjennom FrenixCode-konfigurasjonen.
...
FrenixCode kommer med to innebygde moduser.
...
- `edit` - Kan ikke endre eksisterende filer, bortsett fra filer som ligger på `.frenixcode/plans/*.md` for å detaljere selve planen
...
Konfigurer moduser i din `frenixcode.json` konfigurasjonsfil:
...
- Globalt: `~/.config/frenixcode/modes/`
- Prosjekt: `.frenixcode/modes/`
...
Hvis ingen temperatur er spesifisert, bruker FrenixCode modellspesifikke standardinnstillinger (vanligvis 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
Denne banen er relativ til der konfigurasjonsfilen er plassert. Så dette fungerer for både den globale FrenixCode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Opprett modusfiler i `.frenixcode/modes/` for prosjektspesifikke moduser eller `~/.config/frenixcode/modes/` for globale moduser:

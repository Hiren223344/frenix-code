---
title: Tjener
description: Samhandle med FrenixCode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `frenixcode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en FrenixCode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `frenixcode.local` |
...
Sett `FRENIXCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `frenixcode` som standard, eller sett `FRENIXCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `frenixcode serve` og `frenixcode web`.
...
Når du kjører `frenixcode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk FrenixCode-serveren til å samhandle med FrenixCode programmatisk.
:::

Denne arkitekturen lar FrenixCode støtte flere klienter og lar deg samhandle med FrenixCode programmatisk.

Du kan kjøre `frenixcode serve` for å starte en frittstående server. Hvis du har
FrenixCode TUI kjører, vil `frenixcode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av FrenixCode [IDE](/docs/ide) plugins.
...
frenixcode-serveren viser følgende APIer.

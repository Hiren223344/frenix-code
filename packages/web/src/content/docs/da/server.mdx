---
title: Server
description: Interager med frenixcode-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `frenixcode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en frenixcode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `frenixcode.local` |
...
Indstil `FRENIXCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `frenixcode`, eller indstil `FRENIXCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `frenixcode serve` og `frenixcode web`.
...
Når du kører `frenixcode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug frenixcode-serveren til at interagere med frenixcode programmatisk.
:::

Denne arkitektur lader frenixcode understøtte flere klienter og lader dig interagere med frenixcode programmatisk.

Du kan køre `frenixcode serve` for at starte en selvstændig server. Hvis du har
frenixcode TUI kørende, vil `frenixcode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af FrenixCode [IDE](/docs/ide) plugins.
...
FrenixCode-serveren viser følgende API'er.

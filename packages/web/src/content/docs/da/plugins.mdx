---
title: Plugins
description: Skriv dine egne plugins for at udvide FrenixCode.
---

Plugins giver dig mulighed for at udvide FrenixCode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre FrenixCodes standardadfærd.
...
- `.frenixcode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/frenixcode/node_modules/`.
...
1. Global konfiguration (`~/.config/frenixcode/frenixcode.json`)
2. Projektkonfiguration (`frenixcode.json`)
3. Global plugin-mappe (`~/.config/frenixcode/plugins/`)
4. Projekt plugin bibliotek (`.frenixcode/plugins/`)
...
FrenixCode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En frenixcode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide frenixcode.
...
:::note
Hvis du bruger FrenixCode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå frenixcode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til frenixcode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som frenixcode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for frenixcode sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://frenixcode.dev/docs/sdk) for detaljer.

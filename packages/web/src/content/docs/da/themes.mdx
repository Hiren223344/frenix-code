...
Med FrenixCode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger FrenixCode vårt eget `frenixcode`-tema.
...
FrenixCode kommer med flere innebygde temaer.
...
- Vil at FrenixCode skal matche terminalens utseende
...
FrenixCode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/frenixcode/themes/*.json` eller `$XDG_CONFIG_HOME/frenixcode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.frenixcode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.frenixcode/themes/*.json`

...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i frenixcode-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i frenixcode giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem frenixcode-konfigurationen.
...
frenixcode leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.frenixcode/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `frenixcode.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger frenixcode modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale frenixcode-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.frenixcode/modes/` for projektspecifikke tilstande eller `~/.config/frenixcode/modes/` for globale tilstande:

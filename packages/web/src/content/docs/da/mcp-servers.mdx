...
Du kan tilføje eksterne værktøjer til FrenixCode ved hjælp af _Model Context Protocol_ eller MCP. FrenixCode understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [FrenixCode Config](https://frenixcode.dev/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/frenixcode`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
FrenixCode håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil FrenixCode:
...
Hvis serveren kræver godkendelse, vil FrenixCode bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `frenixcode mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil FrenixCode opbevare tokens sikkert i `~/.local/share/frenixcode/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem FrenixCode-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde FrenixCode til din Sentry-konto.

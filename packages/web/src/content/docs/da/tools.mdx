...
Her er alle de innebygde verktøyene som er tilgængelige i FrenixCode.
...
:::note
Dette verktøyet er bare tilgængelig når `FRENIXCODE_EXPERIMENTAL_LSP_TOOL=true` (eller `FRENIXCODE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger FrenixCode-leverandøren eller når miljøvariabelen `FRENIXCODE_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).

...
Der er to typer agenter i FrenixCode; primære agenter og subagenter.
...
FrenixCode leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
FrenixCode leveres med to indbyggede underagenter, **Generelt** og **Udforsk**. Vi vil se på dette nedenfor.

---

## Indbyggede

FrenixCode leveres med to indbyggede primære agenter og to indbyggede subagenter.
...
Konfigurer agent i din `frenixcode.json`-konfigurationsfil:
...
Hvis ingen temperatur er angivet, bruger FrenixCode modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale FrenixCode-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din FrenixCode-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [FrenixCode Zen](/docs/zen), vil du bruge `frenixcode/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `frenixcode models` for at se en liste over de tilgængelige modeller.
:::
...
:::tip
Har du en agent, du gerne vil dele? [Send en PR](https://github.com/Hiren223344/frenix-code).
:::

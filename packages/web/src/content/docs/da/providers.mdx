---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i FrenixCode.
---

import config from "../../../../config.mjs"
export const console = config.console

FrenixCode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i FrenixCode-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/frenixcode/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i FrenixCode-konfigurationen.
...
FrenixCode Zen er en liste over modeller leveret af FrenixCode-teamet, som er blevet testet og verificeret til at fungere godt med FrenixCode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med FrenixCode Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg frenixcode og gå til [frenixcode.dev/auth](https://frenixcode.dev/auth).
...
Det fungerer som alle andre udbydere i FrenixCode og er helt valgfrit at bruge.

---

## FrenixCode Go

FrenixCode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af FrenixCode-teamet, som er testet og verificeret til at fungere godt med FrenixCode.

1. Kør kommandoen `/connect` i TUI, vælg `FrenixCode Go`, og gå til [frenixcode.dev/auth](https://frenixcode.dev/zen).
...
Sådan bruger du Amazon Bedrock med FrenixCode:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører frenixcode:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `frenixcode.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i FrenixCode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at frenixcode skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem frenixcode-konfigurationen.
...
gemme tokenet i frenixcode auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
FrenixCode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse FrenixCode
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`frenixcode.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med FrenixCode:
...
Sådan bruger du Google Vertex AI med FrenixCode:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører frenixcode.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem frenixcode, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [frenixcode-helicone-session](https://github.com/H2Shami/frenixcode-helicone-session) plugin for automatisk at logge hver FrenixCode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver FrenixCode-samtale opført som en separat session.
...
Du kan konfigurere frenixcode til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere frenixcode til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere frenixcode til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for FrenixCode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/frenixcode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med FrenixCode:
...
3. Kopier API-nøglen til brug i FrenixCode.
...
6. **Vigtigt**: Før du bruger skymodeller i FrenixCode, skal du hente modelinformationen lokalt:
...
FrenixCode Zen er en liste over testede og verificerede modeller leveret af FrenixCode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>FrenixCode Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **FrenixCode Zen**.

   ```txt
   /connect
   ```

3. Indtast FrenixCode API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem frenixcode-konfigurationen.
...
5. Du kan også tilpasse dem gennem frenixcode-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med FrenixCode:
...
Du kan også tilpasse modeller gennem frenixcode-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med frenixcode. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `frenixcode.json`-filen i projektkataloget:
...
`limit`-felterne lader FrenixCode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `frenixcode auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek frenixcode-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i frenixcode-konfigurationen.

---
title: Konfiguration
description: Ved at bruge FrenixCode JSON konfig.
---

Du kan konfigurere FrenixCode ved hjælp af en JSON-konfigurationsfil.

---

## Format

FrenixCode understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/frenixcode`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/frenixcode/frenixcode.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`FRENIXCODE_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`frenixcode.json` i projekt) - projektspecifikke indstillinger
5. **`.frenixcode` mapper** - agent, kommandoer, plugins
6. **Inline config** (`FRENIXCODE_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.frenixcode` og `~/.config/frenixcode` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/frenixcode`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale FrenixCode-konfiguration i `~/.config/frenixcode/frenixcode.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `frenixcode.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når FrenixCode starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `FRENIXCODE_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `FRENIXCODE_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.frenixcode` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.frenixcode` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`frenixcode.dev/config.json`**](https://frenixcode.dev/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `frenixcode serve` og `frenixcode web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din FrenixCode-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `frenixcode.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din FrenixCode-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger FrenixCode at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din FrenixCode-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/frenixcode/agents/` eller `.frenixcode/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil FrenixCode falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`frenixcode run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/frenixcode/commands/` eller `.frenixcode/commands/`. [Learn more here](/docs/commands).
...
FrenixCode vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard frenixcode **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide FrenixCode med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.frenixcode/plugins/` eller `~/.config/frenixcode/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse FrenixCode til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.

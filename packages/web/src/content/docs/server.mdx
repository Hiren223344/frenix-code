---
title: Server
description: Interact with frenixcode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `frenixcode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an frenixcode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `frenixcode.local` |
...
Set `FRENIXCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `frenixcode`, or set `FRENIXCODE_SERVER_USERNAME` to override it. This applies to both `frenixcode serve` and `frenixcode web`.
...
When you run `frenixcode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the frenixcode server to interact with frenixcode programmatically.
:::

This architecture lets frenixcode support multiple clients and allows you to interact with frenixcode programmatically.

You can run `frenixcode serve` to start a standalone server. If you have the
frenixcode TUI running, `frenixcode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the FrenixCode [IDE](/docs/ide) plugins.
...
The frenixcode server exposes the following APIs.

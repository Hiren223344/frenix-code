...
مع FrenixCode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم FrenixCode سمتنا `frenixcode`.
...
يأتي FrenixCode مع عدة سمات مدمجة.
...
- يريدون أن يطابق FrenixCode مظهر terminal لديهم
...
يدعم FrenixCode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/frenixcode/themes/*.json` أو `$XDG_CONFIG_HOME/frenixcode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.frenixcode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.frenixcode/themes/*.json`

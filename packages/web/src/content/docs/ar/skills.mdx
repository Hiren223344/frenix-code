...
تتيح مهارات الوكيل لـ FrenixCode اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث FrenixCode في هذه المواقع:

- إعدادات المشروع: `.frenixcode/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/frenixcode/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج FrenixCode صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.frenixcode/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/frenixcode/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.frenixcode/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض FrenixCode المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `frenixcode.json`:
...
**للوكلاء المدمجين** (في `frenixcode.json`):

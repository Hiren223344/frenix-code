---
title: الخادم
description: تفاعل مع خادم frenixcode عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `frenixcode serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل frenixcode استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `frenixcode.local` |
...
عيّن `FRENIXCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `frenixcode`، أو عيّن `FRENIXCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `frenixcode serve` و `frenixcode web`.
...
عند تشغيل `frenixcode` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم frenixcode للتفاعل مع frenixcode برمجيا.
:::

تتيح هذه البنية لـ frenixcode دعم عدة عملاء وتمكّنك من التفاعل مع frenixcode برمجيا.

يمكنك تشغيل `frenixcode serve` لبدء خادم مستقل. إذا كانت واجهة frenixcode في terminal (TUI)
قيد التشغيل، فسيبدأ `frenixcode serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات FrenixCode لـ [IDE](/docs/ide).
...
يعرض خادم frenixcode واجهات API التالية.

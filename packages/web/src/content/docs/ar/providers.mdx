---
title: المزوّدون
description: استخدام أي موفّر LLM في FrenixCode.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم FrenixCode ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات FrenixCode.
...
في `~/.local/share/frenixcode/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات FrenixCode.
...
FrenixCode Zen هي قائمة نماذج يوفّرها فريق FrenixCode وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع FrenixCode. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع FrenixCode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر frenixcode، ثم انتقل إلى [frenixcode.dev/auth](https://frenixcode.dev/auth).
...
يعمل مثل أي مزوّد آخر في FrenixCode واستخدامه اختياري بالكامل.

---

## FrenixCode Go

FrenixCode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق FrenixCode، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع FrenixCode.

1. شغّل الأمر `/connect` في TUI، واختر `FrenixCode Go`، ثم انتقل إلى [frenixcode.dev/auth](https://frenixcode.dev/zen).
...
لاستخدام Amazon Bedrock مع FrenixCode:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل frenixcode:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `frenixcode.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل FrenixCode غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل frenixcode بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات frenixcode.
...
تخزين الرمز في مساحة تخزين مصادقة frenixcode.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم FrenixCode نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر FrenixCode
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`frenixcode.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع frenixcode:
...
لاستخدام Google Vertex AI مع FrenixCode:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر frenixcode، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [frenixcode-helicone-session](https://github.com/H2Shami/frenixcode-helicone-session) لتسجيل كل محادثة FrenixCode تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة FrenixCode مدرجة كجلسة مستقلة.
...
يمكنك ضبط frenixcode لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط frenixcode لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط frenixcode لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع FrenixCode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/frenixcode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع FrenixCode:
...
3. انسخ مفتاح API لاستخدامه في FrenixCode.
...
6. **مهم**: قبل استخدام نماذج السحابة في FrenixCode، يجب سحب معلومات النموذج محليا:
...
FrenixCode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق FrenixCode. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>FrenixCode Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **FrenixCode Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ FrenixCode.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات frenixcode.
...
5. يمكنك أيضا تخصيصها عبر إعدادات frenixcode. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات frenixcode. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع frenixcode. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `frenixcode.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ FrenixCode بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `frenixcode auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات frenixcode وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات frenixcode.

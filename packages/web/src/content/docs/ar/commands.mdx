...
أنشئ الملف `.frenixcode/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات FrenixCode أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ FrenixCode:
...
- على مستوى النظام: `~/.config/frenixcode/commands/`
- لكل مشروع: `.frenixcode/commands/`
...
يتضمن frenixcode عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).

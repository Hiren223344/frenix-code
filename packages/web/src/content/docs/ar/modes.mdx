...
:::caution
يتم الآن ضبط الأوضاع عبر خيار `agent` في إعدادات frenixcode. أصبح خيار
`mode` مُهمَلًا الآن. [اعرف المزيد](/docs/agents).
:::

تتيح لك الأوضاع في frenixcode تخصيص السلوك والأدوات والمطالبات لحالات استخدام مختلفة.

يأتي مع وضعين مدمجين: **build** و **plan**. يمكنك تخصيصهما أو إعداد
أوضاعك الخاصة عبر إعدادات frenixcode.
...
يأتي frenixcode مع وضعين مدمجين.
...
- `edit` - لا يمكن تعديل الملفات الموجودة، باستثناء الملفات الموجودة في `.frenixcode/plans/*.md` لتفصيل الخطة نفسها
...
قم بإعداد الأوضاع في ملف الإعدادات `frenixcode.json` لديك:
...
- عالمي: `~/.config/frenixcode/modes/`
- للمشروع: `.frenixcode/modes/`
...
إذا لم يتم تحديد درجة الحرارة، يستخدم frenixcode القيم الافتراضية الخاصة بكل نموذج (عادةً 0 لمعظم النماذج و 0.55 لنماذج Qwen).
...
إعدادات frenixcode العالمية وكذلك إعدادات المشروع.
...
أنشئ ملفات الأوضاع في `.frenixcode/modes/` لأوضاع خاصة بالمشروع أو في `~/.config/frenixcode/modes/` لأوضاع عالمية:

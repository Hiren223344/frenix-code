...
const deepLinkEvent = "frenixcode:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__FRENIXCODE__ ??= {}
  const pending = window.__FRENIXCODE__.deepLinks ?? []
  window.__FRENIXCODE__.deepLinks = [...pending, ...urls]
  window.dispatchEvent(new CustomEvent(deepLinkEvent, { detail: { urls } }))
}

const listenForDeepLinks = () => {
  const startUrls = window.__FRENIXCODE__?.deepLinks ?? []
...
const createPlatform = (): Platform => {
  ...
  const wslHome = async () => {
    if (os !== "windows" || !window.__FRENIXCODE__?.wsl) return undefined
    return window.api.wslPath("~", "windows").catch(() => undefined)
  }

  const handleWslPicker = async <T extends string | string[]>(result: T | null): Promise<T | null> => {
    if (!result || !window.__FRENIXCODE__?.wsl) return result
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://frenixcode.dev/favicon-96x96-v3.png",
      })
    ...
    getWslEnabled: async () => {
      ...
      return window.__FRENIXCODE__!.wsl ?? false
    },

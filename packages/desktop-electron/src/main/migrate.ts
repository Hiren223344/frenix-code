...
const TAURI_APP_IDS: Record<string, string> = {
  dev: "ai.frenixcode.desktop.dev",
  beta: "ai.frenixcode.desktop.beta",
  prod: "ai.frenixcode.desktop",
}
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.frenixcode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `frenixcode.settings.dat` is special: it maps to the `frenixcode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"frenixcode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // frenixcode.settings.dat → the electron settings store ("frenixcode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "frenixcode.global.dat").
  const storeName = filename === "frenixcode.settings.dat" ? "frenixcode.settings" : filename

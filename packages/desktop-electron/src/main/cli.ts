...
const CLI_INSTALL_DIR = ".frenixcode/bin"
const CLI_BINARY_NAME = "frenixcode"
...
export function getSidecarPath() {
  ...
  const path = app.isPackaged
    ? join(process.resourcesPath, `frenixcode-cli${suffix}`)
    : join(root, "../../resources", `frenixcode-cli${suffix}`)
...
export async function installCli(): Promise<string> {
  ...
  const tempScript = join(tmpdir(), "frenixcode-install.sh")
...
export function serve(hostname: string, port: number, password: string) {
  ...
  const env = {
    FRENIXCODE_SERVER_USERNAME: "frenixcode",
    FRENIXCODE_SERVER_PASSWORD: password,
  }
...
export function spawnCommand(args: string, extraEnv: Record<string, string>) {
  ...
  const envs = {
    ...base,
    FRENIXCODE_EXPERIMENTAL_ICON_DISCOVERY: "true",
    FRENIXCODE_EXPERIMENTAL_FILEWATCHER: "true",
    FRENIXCODE_CLIENT: "desktop",
...
function buildCommand(args: string, env: Record<string, string>) {
  if (process.platform === "win32" && isWslEnabled()) {
    ...
    const script = [
      "set -e",
      'BIN="$HOME/.frenixcode/bin/frenixcode"',
      'if [ ! -x "$BIN" ]; then',
      `  curl -fsSL https://frenixcode.dev/install | bash -s -- --version ${shellEscape(version)} --no-modify-path`,
      "fi",

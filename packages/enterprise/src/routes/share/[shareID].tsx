import { FileDiff, Message, Model, Part, Session, SessionStatus, UserMessage } from "@frenixcode/sdk/v2"
import { SessionTurn } from "@frenixcode/ui/session-turn"
import { SessionReview } from "@frenixcode/ui/session-review"
import { DataProvider } from "@frenixcode/ui/context"
import { FileComponentProvider } from "@frenixcode/ui/context/file"
import { WorkerPoolProvider } from "@frenixcode/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createMemo, createSignal, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@frenixcode/ui/logo"
import { IconButton } from "@frenixcode/ui/icon-button"
import { ProviderIcon } from "@frenixcode/ui/provider-icon"
import { iife } from "@frenixcode/util/iife"
import { Binary } from "@frenixcode/util/binary"
import { NamedError } from "@frenixcode/util/error"
import { DateTime } from "luxon"
import { createStore } from "solid-js/store"
import z from "zod"
import NotFound from "../[...404]"
import { Tabs } from "@frenixcode/ui/tabs"
import { MessageNav } from "@frenixcode/ui/message-nav"
import { FileSSR } from "@frenixcode/ui/file-ssr"
...
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@frenixcode/ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/frenixcode-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | FrenixCode</Title>
              </Show>
              <Meta name="description" content="frenixcode - The AI coding agent built for the terminal." />
                        ...
                        <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                          <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                            ...
                            <div class="flex gap-3 items-center">
                              <IconButton
                                as={"a"}
                                href="https://github.com/Hiren223344/frenix-code"
                                target="_blank"
                                icon="github"
                                variant="ghost"
                              />
                              <IconButton
                                as={"a"}
                                href="https://frenixcode.dev/discord"
                                target="_blank"
                                icon="discord"
                                variant="ghost"
                              />

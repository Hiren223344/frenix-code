...
const reg = process.env.REGISTRY ?? "ghcr.io/frenixcode"
...
const setup = async () => {
  ...
  if (list.includes("frenixcode")) {
    await $`docker buildx use frenixcode`
    return
  }
  await $`docker buildx create --name frenixcode --use`
}

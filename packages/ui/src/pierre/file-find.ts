...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("frenixcode-find")
  api.delete("frenixcode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("frenixcode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("frenixcode-find", new Highlight(...rest))
    return true
  }

...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("frenixcode-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("frenixcode_lib=debug,frenixcode_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("frenixcode_lib=info,frenixcode_desktop=info,sidecar=info")
        }
    });

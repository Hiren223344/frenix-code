...
async fn initialize(app: AppHandle) {
    ...
    let _ = ready_tx.send(ServerReadyData {
        url: url.clone(),
        username: Some("frenixcode".to_string()),
    ...
    let sqlite_done = needs_migration.then(|| {
        tracing::info!(
            path = %frenixcode_db_path().expect("failed to get db path").display(),
...
fn sqlite_file_exists() -> bool {
    let Ok(path) = frenixcode_db_path() else {
        return true;
    };
...
fn frenixcode_db_path() -> Result<PathBuf, &'static str> {
    ...
    Ok(data_home.join("frenixcode").join("frenixcode.db"))
}

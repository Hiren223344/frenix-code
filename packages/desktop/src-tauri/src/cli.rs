...
const CLI_INSTALL_DIR: &str = ".frenixcode/bin";
const CLI_BINARY_NAME: &str = "frenixcode";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("frenixcode-install.sh");
...
pub fn spawn_command(
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut script = vec![
                "set -e".to_string(),
                "BIN=\"$HOME/.frenixcode/bin/frenixcode\"".to_string(),
                "if [ ! -x \"$BIN\" ]; then".to_string(),
                format!(
                    "  curl -fsSL https://frenixcode.dev/install | bash -s -- --version {} --no-modify-path",
...
pub fn serve(
    ...
    let envs = [
        ("OPENCODE_SERVER_USERNAME", "frenixcode".to_string()),
    ...
    .expect("Failed to spawn frenixcode");

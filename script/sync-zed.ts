...
const FORK_REPO = "frenixcode/zed-extensions"
const UPSTREAM_REPO = "zed-industries/extensions"
const EXTENSION_NAME = "frenixcode"
...
async function main() {
  ...
  // Delete any existing branches for frenixcode updates
  ...
  const prResult =
    await $`gh pr create --repo ${UPSTREAM_REPO} --base main --head ${FORK_REPO.split("/")[0]}:${branchName} --title "Update ${EXTENSION_NAME} to v${cleanVersion}" --body "Updating FrenixCode extension to v${cleanVersion}"`

...
async function fix(pr: PR, files: string[]) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with frenixcode...`)
  const prompt = [
  ...
  try {
    await $`frenixcode run -m frenixcode/gpt-5.3-codex ${prompt}`
  } catch (err) {
    console.log(`  frenixcode failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with frenixcode")
  return true
}
